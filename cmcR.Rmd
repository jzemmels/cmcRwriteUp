...
$$
Note that this finite, discretized CCF definition yields a matrix of elements representing the similarity between matrices $A$ and $B$ for various translations of matrix $B$. The index at which the CCF attains a maximum represents the translations needed to align $B$ with $A$.

\jz{Discuss ambiguity in how the CCF is to be calculated. Calculation based on the definition is computationally infeasible, but there isn't a clear indication in any of the main articles from which we've based the package how the CCF is calculated. \citet{ott_applying_2017} and \citet{tong_valid_2018} are the only papers I have found that define the "Areal Cross Correlation Function" using actual, clear notation.}

\jz{There is no clear indication that the FFT-based method is used in any of the main papers other than the CCF maps in \citet{tong_improved_2015} (which I guess *could* have been created based on calculating the CCF from the definition). I think this section will need to be less quote-heavy since there's such ambiguity in how the CCF is actuall computed.}

\jz{In the cmcR package implementation description, discuss how/why the CCF is calculated using the FFT-based method followed by the pairwise complete correlation.}

## The initially proposed CMC method

"The qualiﬁcations of CMCs require not only high correlation values CCF$_{\max} \geq T_{\text{CCF}}$, but also similar registration angles $\theta$ (within the threshold $T_{\theta}$) and similar $x–y$ registration pattern (within the thresholds $T_x$, $T_y$)."

"a virtual reference with three reference registration parameters $\theta_{\text{ref}}$, $x_{\text{ref}}$ and $y_{\text{ref}}$ generated by the median values of the collective $\theta$, and $x$-, $y$-translation values of all cell pairs in each correlation sequence."
...
\jz{This section is currently (6/14/2020) being re-worked above.}
