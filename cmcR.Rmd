...
\dfn{Research reproducibility} commonly refers to the ability to use the same materials and procedures as a previous study to arrive at the same results or conclusions. In computationally-intensive research, this is commonly translated to mean that the code and data used to produce results be made openly available \citep{peng_reproducible_2011}. Reproducibility is necessary for results to be considered valid by the wider scientific community (cite Goodman et al.). However, recent studies have raised concerns about reproducibility of scientific findings in various fields [cite Pasquier et al., \citet{baker_1500_2016}, King]. Commonly identified reasons for this lack of reproducibility include ambiguity in how a procedure was implemented and inaccessible data or code (cite Leek and Jager). Some journals have adopted policies encouraging or requiring that data and code be made available in an effort to build a "culture of reproducibility" in their respective fields [\citet{peng_reproducible_2011}, cite Peng and Zeger, Stodden et al.].

Computational reproducibility poses additional challenges. Published results may be numerically sensitive to the particular implementation of a computational method including such considerations as data processing decisions, parameter settings, and even the chosen programming language [\citep{peng_reproducible_2011},\citet{Stodden2013SettingTD}]. Recently developed tools allow for greater clarity in how such decisions are made so that less time need be wasted double-checking how close reproduced output matches to published results [cite ICERM workshop paper].

The development of computationally-intensive tools to assist forensic practitioners in making informed decisions has recently grown in interest \citep{council_strengthening_2009}. Many methods have been developed to handle problems such as identifying matching glass shards, shoe prints, or ballistic signatures. While many researchers in the forensic science community have adopted computationally reproducible habits [cite CSAFE stuff], there are notable exceptions (make this more/less pointed?). The Congruent Matching Cells (CMC) algorithm was developed at the National Institute of Standards and Technology in the early 2010s to perform firearm evidence identification using identifiable markings left on spent cartridge cases from a firearm's barrel \citet{song_proposed_2013}. The method and its numerous improvements/extensions indicate promising results in being able to correctly differentiate between matching and non-matching cartridge cases. However, based on the definitions given above, these results are not reproducible; computationally or otherwise. Qualitative reproduction of results has successfully been done among various NIST researchers. However, code for the CMC method or any of its improvements/extensions has been made openly available. Further, many procedures used in the CMC method are described qualitatively rather than algorithmically; meaning they do not sufficiently delve into the procedures' intricacies (e.g., parameter settings). Especially in applications like forensic science where results from computational methods may affect legal decisions, transparency in how a method or algorithm is implemented is necessary [\citep{angwin_machine_2016}, cite Cino].

The \pkg{cmcR} package not only provides a open-source implementation of the CMC method, but also serves as an exemplar of what must be done to implement a method for which only a qualitative description is available. To be clear, we do not intend to criticize the quality of work of particular authors or research teams. Rather, we hope to use the \pkg{cmcR} package to discuss themes and issues surrounding reproducibility that pervade many domains of computationally-intensive research.
