...
The 1993 Supreme Court case \citetalias{daubert} established the \`\`Daubert Standard" which requires that a scientific method have a known error rate and be widely accepted by a relevant scientific community before it can be used in an expert witness's testimony. To achieve this standard for forensic pattern-matching algorithms, we need to be able to unambiguously reproduce the results of the algorithm. \\svp{This doesn't flow that well because we used a different form of the word reproducibility... fix? rephrase?} \dfn{Reproducibility} refers to the ability to use the same procedures and materials as a previous study to arrive at the same results or conclusions. Unfortunately, we cannot take reproducibility for granted: recent studies have raised concerns about reproducibility of scientific findings in various fields \citep{king_replication_1995,baker_1500_2016,pasquier_if_2017}. \jz{While a reproduction of results by the original research group is the bare minimum to achieve scientific rigor, it is not particularly helpful in consequential fields like forensic science where reproduction of results by other research groups or practitioners is important.} In computational research, reproducibility of results is commonly translated \svp{used?} to mean that the code and data used to produce published results be made publicly available \citep{peng_reproducible_2011}. <!-- \hh{XXX we also need to talk about 'who is able to reproduce' - reproducibility of research by the research group who published the finding should be considered a minimum of scientific rigor, but it is not particularly helpful in a high-stakes field such as forensic science.} --> Computational reproducibility requires more details: published results may be numerically sensitive to the particular implementation of a computational method, including such considerations as data processing decisions, parameter settings, and even the chosen programming language. \svp{We may need to rephrse this -- we're saying "in computational research, reproducibility means..." and then using the phrase "computational reproducibility" to mean something different} In this light, computational reproducibility in forensic science is all the more important because it enables discussions about algorithmic transparency and accountability \citep{kwongAlgorithmSaysYou2017, desaiTrustVerifyGuide2017}. Therefore, it is of the utmost importance \\svp{too many uses of importan\\\*} that the forensic community does not go only halfway, trading a subjective, human black box for objective, proprietary algorithms that are similarly opaque and unauditable. As a community, we need to develop our algorithms from the ground up with the intention that any results be immediately computationally reproducible by \jz{interested parties, such as another research group,} rather than eventually qualitatively reproducible. \svp{you haven't introduced the by whom question, so this is a bit disjoint}
