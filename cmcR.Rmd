...
plt1 <- plt1  +
  ...
  geom_text(data = data.frame(x3p = c("Fadul 1-1","Fadul 1-2"),
                              x = c(1750,1750),
                              y = c(1500,1500)),
            aes(x = x,y = y,label = x3p),size = 2.5)
...
<!-- \jz{Changes to the pre-processing procedures have a major impact on the effectiveness of the CMC method.  -->
...
<!-- Recall that \jz{the last step of the [High CMC method](#highCMCMethod) is to combine CMCs from the two comparison directions, assuming that the High CMC criteria were satisfied. -->
...
\svp{Both the CMC and High CMC methods are complex, multi-step processes that are easier to run and debug when written as a modular pipeline of successive, functionally independent steps.
...
This provides a huge boost to the efficiency of new developments in the field: it is our hope that the cmcR package will facilitate additional research into the matching of cartridge cases and other pattern evidence.}


\svp{As described in \citet{wittenburg_open_2021}, open science and data science depend on FAIR algorithms and data, where FAIR stands for findability, accessibility, interoperability, and reusability. 
...
\svp{FAIR software standards emphasize open code, but it is just as important in data science applications to have open data. 
