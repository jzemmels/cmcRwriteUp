...
In this paper, we will demonstrate the ambiguities present in textual descriptions of algorithms by creating an open-source implementation of one well-known automatic forensic identification method. <!-- intentional buttering up before the punchline --> 
...
As a result, \hh{the findings described in these papers are not computationally reproducible for the research community at large.}
...
Unsurprisingly, \hh{this process is both} time \hh{ absorbing and computationally} intensive from implementation to sifting through a wide variety of decision combinations. <!--This stymies scientific progress.-->
\hh{The failure to ensure computational reproducibility} not only imposes a general barrier to scientific progress; in this application, it also highlights the \hh{crucial need} of open-source, transparent methodology in high-stakes fields such as forensic analysis.
...
It strikes the back wall, known as the \dfn{breech face}, of the barrel with considerable force, \hh{thereby imprinting any markings on the breech face onto the cartridge case, creating} the so-called \dfn{breech face impressions}.
\hh{These markings have been suggested to be unique to a firearm and are used in forensic examinations to determine whether two cartridge cases have been fired by the same handgun.}

During a forensic examination, \hh{two pieces of ballistic evidence} are  placed under a \dfn{comparison microscope}. Comparison microscopes allow for a \hh{side-by-side} comparison of two objects within the same viewfinder, as seen in \ref{fig:ccPair_combined}. \hh{A pair of breech face images is aligned along the thin black line in the middle of the images. }
<!--  \hh{taking the first image out - we don't want to be side tracked.}
Figures \ref{fig:ccPair_separated} and
\ref{fig:ccPair_combined}, which show two pairs of breech faces. In \autoref{fig:ccPair_separated} the pair is unaligned, in \autoref{fig:ccPair_combined} the cartridge cases are aligned. 
The similarity in the markings between the two breech face impressions  is brought out very clearly when they are aligned.
...
\autoref{fig:ccPair_combined} \hh{is an example of an image coming from} a comparison microscope used by firearms examiners in laboratory settings. Digital microscopy is capable of   precision measurements of surface topology \hh{at even higher resolutions}. Using a 3D microscope, we can obtain scans of breech face impressions \hh{at the micron level ($1 \mu m = 10^{-3} mm = 10^{-6} m$.) 
...
Cartridge case scans are commonly stored in the ISO standard x3p file format \citep{ISO25178-72}. An x3p consists of a single surface matrix representing the height value of the surface at a particular location and metadata concerning the parameters under which the scan was taken (size, resolution, creator, microscope, and microscopy software versions etc.). The \pkg{x3ptools} package \citep{x3ptools} can be used to work with these x3p files in R \citep{x3ptools}.
...
> Trim off the inside firing pin surface and other areas outside the breed (sic) face mark, so that only breech face impression data remain for correlation.
...
fadul1.1 <- x3ptools::x3p_read("data/fadul1-1.x3p") ## Could we include the code in the figure?
```
...
Step 1 in \autoref{fig:processingPipeline} shows an \jz{unprocessed surface matrix.} \hh{XXX can we get away from the surface matrix and call it a breech face scan? That's gettign away from how it's implemented towards what it is.}
\hh{XXX minor thing, but let's call the things exactly how they are defined. It's not Step 1, its (1) in Figure 5.}

```{r, echo=TRUE,eval=FALSE}
## HH: Couple of questions: where do the -30 and the 200 come from?
## Why is it *crop*Exterior and *filter*Interior?
## It looks like both functions have the same parameterization
## how about a function preProcess_crop with a parameter area that is "interior" or "exterior"?
## is it that we are dealing with a radius? could the parameter be just offset?
...
As seen in step 1 of \autoref{fig:processingPipeline}, the center of the firing pin impression hole might be offset from the center of the cartridge case.
The estimation procedures systematically overestimate and underestimate the radii of the cartridge case primer and firing pin impression regions, respectively (e.g., the estimation procedure captures roll-off on on the exterior of the primer).
The user-specified `radiusOffset` is added to the radius estimate to improve a radius estimate.
...
The \code{preProcess\_crop} function is used to remove the exterior and firing pin impression region on the interior based on the `region` argument.}
...
The band-pass filtered surface matrix as returned by the \code{preProcess\_gaussFilter} function is shown in the fifth panel of \autoref{fig:processingPipeline}.
...
\jz{\autoref{fig:processedScans} displays the processed Fadul 1-1 and Fadul 1-2 scans}; \svp{the second matrix is processed using the same function arguments}. \jz{A processed pair of cartridge cases are used as the input to the next stage of the CMC methos, the cell-based comparison procedure.}
...
Similar to the \code{preProcess\_*} functions, the \code{comparison\_*} functions were designed to be used with the pipe operator.}
...
- \svp{The optimal translation is determined using the FFT-based CCF. Replacing the missing values with the overal mean leads to a deflated CCF value (see appendix XXX for more details), but produces an accurate estimate of the optimal translation \jz{(via the \code{comparison\_fft\_ccf} function)}.}
...
For a particular $\theta$, let CMC$_{\theta}$ be the number of votes that are deemed "close enough" to the reference values $x_{\text{ref},\theta}, y_{\text{ref},\theta}$.
Combining the CMC$_{\theta}$ values across all $\theta$ values considered in the comparison yields what \citet{tong_improved_2015} refer to as a "CMC-$\theta$" distribution.
A single cell/region pair may be considered congruent for multiple $\theta$ values.
While there should only be one "true" $\theta$ alignment value, \citet{tong_improved_2015} justify their method by the empirical observation:}
...
\jz{If the angular range is within $T_\theta$, we say that the cartridge case pair "passes" the High CMC criterion. Otherwise, the pair "fails" the High CMC criterion and the CMCs determined under the original method \citet{song_proposed_2013} are used.}
...
\jz{The steps of the decision rule stage are implemented via the \code{decision\_*} functions. In particular, the \code{decision\_CMC} function applies both the decision rule of the original method of \citet{song_proposed_2013} and the High CMC method depending on whether the user provides a value for $\tau$, the constant used to define "high" CMC values in the CMC-$\theta$ distribution. The code below demonstrates how the \code{decision\_CMC} can be used within a \code{mutate} call to add columns to the \code{kmComparisonFeatures} data frame. We also compute the CMCs under both decision rules for the \code{kmComparisonFeatures\_rev} data frame consisting of the similarity features for the comparison of Fadul 1-2 vs. Fadul 1-1. All 3 stages of the CMC method are also repeated for the comparison between the non-match pair Fadul 1-1 and Fadul 2-1, although none of the executed code is shown to avoid redundancy.}
...
km_originalCMCPlot_bothDirections <- ggplot(data.frame(a = 1)) +
  ...
  annotation_custom(ggplotGrob(km_originalCMC_reference_v_target),
                    xmin = 1,xmax = 10,ymin = 5.15,ymax = 10) +
  annotation_custom(ggplotGrob(km_originalCMC_target_v_reference),
                    xmin = 1,xmax = 10,ymin = 1,ymax = 5.15) +
  annotation_custom(kmLegend_originalCMC,
                    xmin = 9,xmax = 9,ymin = 5.5,ymax = 5.5) +
  annotate("text",x = 4.35,y = 7.15,size = 5,label = "Fadul 1-1") +
  annotate("text",x = 4.35,y = 3,size = 5,label = "Fadul 1-1") +
  annotate("text",x = 6.55,y = 7.15,size = 5,label = "Fadul 1-2") +
  annotate("text",x = 6.55,y = 3,size = 5,label = "Fadul 1-2")
```
...
km_highCMCCMCPlot_bothDirections <- ggplot(data.frame(a = 1)) +
  ...
  annotation_custom(ggplotGrob(km_highCMC_reference_v_target),
                    xmin = 1,xmax = 10,ymin = 5.55,ymax = 10) +
  annotation_custom(ggplotGrob(km_highCMC_target_v_reference),
                    xmin = 1,xmax = 10,ymin = 1,ymax = 5.55) +
  annotation_custom(kmLegend_highCMC,
                    xmin = 9,xmax = 9,ymin = 5.5,ymax = 5.5) +
  annotate("text",x = 4.25,y = 7.5,size = 5,label = "Fadul 1-1") +
  annotate("text",x = 4.25,y = 3,size = 5,label = "Fadul 1-1") +
  annotate("text",x = 6.6,y = 7.5,size = 5,label = "Fadul 1-2") +
  annotate("text",x = 6.6,y = 3,size = 5,label = "Fadul 1-2")
```
...
knm_cmcPlot_bothDirections <- ggplot(data.frame(a = 1)) +
  ...
  annotation_custom(knmLegend,
                    xmin = 9,xmax = 9,ymin = 5.5,ymax = 5.5) +
  annotate("text",x = 4.45,y = 7.65,size = 5,label = "Fadul 1-1") +
  annotate("text",x = 4.25,y = 3,size = 5,label = "Fadul 1-1") +
  annotate("text",x = 6.75,y = 7.65,size = 5,label = "Fadul 2-1") +
  annotate("text",x = 6.75,y = 3.25,size = 5,label = "Fadul 2-1")
```
...
There is no discussion in \citet{song_proposed_2013} about how to combine the results from these two comparison directions, but \citet{tong_improved_2015} propose using the minimum of the two CMC counts (17 in this example).}

```{r,echo=FALSE,warning=FALSE,message=FALSE,cache = F,fig.cap='\\label{fig:topVoteCMCPlot} CMC results for the comparison between Fadul 1-1 and Fadul 1-2 using the original method of \\citet{song_proposed_2013}. The first row shows the 17 CMCs when Fadul 1-1 is treated as the ``reference" cartridge case to which Fadul 1-2 (the ``target") is compared. The second row shows the 18 CMCs when the roles are reversed. Red cells on the indicate where cells \\emph{not} identified as congruent achieve the maximum pairwise-complete correlation across all rotations of the target scan. It is not discussed in the CMC literature whether the results from both comparison directions are combined for the original method of \\citet{song_proposed_2013}.',fig.align='center',fig.pos='htbp',out.width='\\textwidth'}
...
While not shown, this pair failed the High CMC criterion (in both comparison directions, no less) and thus were assigned 0 CMCs under the High CMC method.}

```{r,echo=FALSE,warning=FALSE,message=FALSE,cache = F,fig.cap='\\label{fig:knmCMCPlot} Applying the High CMC method to the comparison between the non-match pair Fadul 1-1 and Fadul 2-1 results in 1 CMC under the original method of \\citet{song_proposed_2013} (shown above) and 0 CMCs under the High CMC method (not shown). The seemingly random behavior of the red cells, which indicate the translation/rotation at which each cell attained its highest CCF value in the target scan, validates our intuition that the cells in a non-match comparison should not exhibit an observable pattern. Random chance should be the prevailing factor in classifying non-match cells as CMCs.', fig.align='center',fig.pos='htbp',out.width='\\textwidth'}
