...
To determine whether a "prominent peak" exists in the relationship between $\theta$ and CMC$_\theta$, \citet{tong_improved_2015} introduce the "angular range" $R (\tau)$ as an interval of rotation angles covering high CMC$_\theta$ values.  For that, another threshold value $\tau$ is introduced and the range is determined as the interval of angles defined between  $\arg \min_\theta CMC_\theta > CMC_{\text{max}} - \tau$ and 
