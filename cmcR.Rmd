...
Obviously, in many situations, this level of reproducibility is not provided. 
\jz{Many proposed methods do not adhere to the definition of reproducibility in \citet{nas_2019} by providing supplementary open-source code, with some exceptions (e.g., DNA \citep{Tvedebrink2020,osiris}, glass \citep{Curran2000-hp,Park2019}, handwriting \citep{crawford_handwriting_2020}, shoe prints \citep{park_algorithm_2020}, and ballistic evidence \citep{hare_automatic_2016,tai_fully_2018}).}

Instead, we find it useful to consider a more inclusive hierarchy of reproducibility.
\jz{Algorithms at higher tiers of the algorithm are more easily reproducible in the sense that fewer resources are required to implement the algorithm.}
...
\jz{Implementing algorithms based on conceptual descriptions or pseudocode requires enumerating and testing possible parameter choices which, depending on their complexity, can be a lengthy and expensive process.
In contrast, implementing fully reproducible algorithms requires only as much time as it takes to emulate the original coding environment.}
...
The Congruent Matching Cells (CMC) pipeline is a collection of algorithms developed \jz{by a group of researchers} at the National Institute of Standards and Technology (NIST) in 2012 to process and compare cartridge case evidence \citep{song_proposed_2013}. Since then, the pipeline and its numerous extensions \citep{tong_improved_2015,chen_convergence_2017,song_estimating_2018} have shown promise in being able to differentiate between matching and non-matching cartridge cases.
...
\jz{We consider the development of the CMC algorithm to be representative of how many forensic algorithms are developed.
After an algorithm is introduced, researchers build upon the foundation laid by the original algorithm in subsequent papers.
...
<!-- \jz{The numerous publications since the initial CMC method proposal in \citet{song_proposed_2013} reflect that its creators and collaborators are committed to improving the method. -->
...
\jz{XXX This preamble needs to be changed once we iron-out the structure of this section.}
...
\jz{We bring up these ambiguities to demonstrate the difficulties that we faced when translating the conceptual description of the CMC pipeline into an actual pipeline.
A reviewer pointed out that many of these choices shouldn't affect the pipeline drastically.
While this assertion may be true, we believe that any amount of variability that exists solely because of uncertainty in how the method was intended to be implemented is both unnecessary and dangerous in a forensics or legal context.}
...
\jz{For us, the process of re-implementing the comparison and decision steps of the pipeline was fairly straightforward.
...
In the following section, we demonstrate that the ambiguities in the [CMC pipeline descriptions](cmcMethod) and lack of guidance in the choice of parameter settings propagate through the algorithm and produce highly variable results.
...
We focus on a discussion of the sensitivity of some of the pipeline's parameters.
...
\jz{In forensics, many matching algorithms are presented in the form of conceptual descriptions.
...
Consequently, there are multiple choices that satisfy the same vague definition of the algorithm.
This is dangerous in a forensics context because i}f many parameter settings are valid but only a narrow range lead to the same conclusion, it is entirely possible that witnesses for the prosecution and defense may come to different conclusions. 
...
Our contribution to the CMC literature is the open-source implementation, which \jz{fills the gaps} in the human-friendly descriptions in the original papers; we provide more detailed, hardware-friendly code which serves as a companion to the original papers. 
\jz{Our contribution to the forensics community at-large is the modularized pipeline structure used to create \CRANpkg{cmcR}.}
