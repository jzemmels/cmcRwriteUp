...
# kmCMCPlot_list$originalMethodCMCs$`Fadul 1-1`$layers[[2]]$mapping$fill <- NULL

# kmCMCPlot_list$originalMethodCMCs$`Fadul 1-1`$layers[[2]]$mapping$colour <- "black"
# 
# kmCMCPlot_list$originalMethodCMCs$`Fadul 1-1`$layers[[2]]$mapping$size <-
```
...
kmCMCPlot_list$originalMethodCMCs$`Fadul 1-1`$layers <- kmCMCPlot_list$originalMethodCMCs$`Fadul 1-1`$layers[-3]
kmCMCPlot_list$originalMethodCMCs$`Fadul 1-2`$layers <- kmCMCPlot_list$originalMethodCMCs$`Fadul 1-2`$layers[-3]
knmCMCPlot_list$originalMethodCMCs$`Fadul 2-1`$layers <- knmCMCPlot_list$originalMethodCMCs$`Fadul 2-1`$layers[-3]

kmCMCPlot_list$originalMethodCMCs$`Fadul 1-1` <- kmCMCPlot_list$originalMethodCMCs$`Fadul 1-1` +
  ggplot2::theme(legend.position = "none",
                 plot.margin=unit(c(0,-.5,0,-.5), "cm")) +
  coord_fixed(xlim = c(-300,3600), ylim = c(-100,3750))
...
kmCMCPlot_list$originalMethodCMCs$`Fadul 1-1` +
...
kmCMCPlot_list$originalMethodCMCs$`Fadul 1-2` +
  # ggplot2::theme(axis.title.x = ggplot2::element_text(),
  #                axis.text.x = ggplot2::element_text(),
  #                axis.title.y = ggplot2::element_text(),
  #                axis.text.y =ggplot2::element_text(),
...
knmCMCPlot_list$originalMethodCMCs$`Fadul 2-1` +
  # ggplot2::theme(axis.title.x = ggplot2::element_text(),
  #                axis.text.x = ggplot2::element_text(),
  #                axis.title.y = ggplot2::element_text(),
  #                axis.text.y =ggplot2::element_text(),
...
```{r warning=FALSE,message=FALSE,echo=FALSE,out.width='\\textwidth',dpi = 1000,fig.cap = '\\jz{Estimated phase alignment and CCF$_{\\max}$ distributions for the comparison between the matching Fadul 1-1 vs. Fadul 1-2 and non-matching Fadul 1-1 vs. Fadul 2-1. Fadul 1-1 is treated as the "reference" scan, meaning it is split into a grid of cells and compared to regions in the other cartridge cases. The cells shown on Fadul 1-2 and Fadul 2-1 represent where each cell in Fadul 1-1 attains the highest CCF$_{\\max}$ value. The distributions below the scans depict the actual $x,y,\theta$, and CCF$_{\\max}$ values for each cell/region pair used in the comparison. For the first three rows, $x,y,$ and $\theta$ values within the blue ribbons are "congruent" in that they are within the allowed tolerance $(T_x = T_y = 20$ pixels, $T_\\theta = 6$ degree) of the median alignment value. In the last row, CCF$_{\\max}$ values above the allowed threshold $(T_{\\text{CCF}} \\geq .5$) are considered "congruent."}'}
...
gridExtra::grid.arrange(kmCMCPlot_list$originalMethodCMCs$`Fadul 1-1`,
                        kmCMCPlot_list$originalMethodCMCs$`Fadul 1-2`,
                        knmCMCPlot_list$originalMethodCMCs$`Fadul 2-1`,
                        ...
                        blank_label,
                        pltLegend,
                        heights = unit(c(9,3,3,3,3,2),units = "null"),
                        widths = unit(c(1,2,2),"null")
                        # ,layout_matrix = layoutMat
                        )


# cowplot::plot_grid(kmCMCPlot_list$originalMethodCMCs$`Fadul 1-1`,
#                    kmCMCPlot_list$originalMethodCMCs$`Fadul 1-2`,
#                    knmCMCPlot_list$originalMethodCMCs$`Fadul 2-1`,
#                    horizontalTranslation_label,
#                    kmComparison_dxData_plot,
