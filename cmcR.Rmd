...
The convention adopted in the \pkg{cmcR} package is to use the average pixel value as a replacement. \svp{XXX Do we know that they did or did not use a similar consensus in the CMC method? XXX} \jz{XXX The short answer is that we have no idea how they calculated the CCF. I started using the FFT-based method because that's how Xiao Hui calculates the CCF and because it's more common in image registration. However, the CCF values were all quite small, for both matches and non-matches, so we added the step of calculating the pairwise-complete correlation (back in February, I think?). A few papers [\citet{weller_confocal_2012}, \citet{ott_applying_2017}] define the CCF/"Areal Cross-Correlation", as they sometimes like to call it, similar to the one given above, but don't actually discuss how it's implemented. The definition in \citet{ott_applying_2017} indicates that the matrices are normalized by subtracting away the mean pixel intensity and dividing by the standard deviation. This is currently also the default in the \pkg{cmcR} package, so that should probably be discussed below. The definition in \citet{weller_confocal_2012} does not show the same normalization. The definition given for the ACCF in those two papers referenced are similar to one in \cite{vorburger_surface_2007} who say "it is advantageous to make use of frequency domain methods to speed up the computation of the similarity metric" (page 54). They discuss using proprietary software from a company called Intelligent Automation, Inc., but they don't go any further than saying that frequency domain methods are used. XXX}
...
\jz{Based on the definition, the CCF need not be bounded between $-1$ and $1$. However, it is common to normalize the CCF for interpretability and is done in the \pkg{cmcR} package.}

\jz{XXX Also describe how the calculation of the "Areal Cross-Correlation" in some NIST papers appears to normalize the surface matrices (based on some definitions). Normalization is currently done by default in the \pkg{cmcR} package (each cell/region are independently normalized), but this can be changed by the user.XXX}
