...
\svp{Initially, we began by comparing the published CMC papers to develop the preprocessing and decision rule charts shown in Figures \ref{fig:preprocessing-schematic} and \ref{fig:processingPipeline}.
...
\svp{While none of these factors are unique to the CMC algorithm, what is unique is the particular combination of an algorithm with many parameters that is extremely sensitive to the parameter settings and the gravity of the material the algorithm is intended to process.
...
```{=tex}
\svp{A completely reproducible paper requires not only algorithm descriptions, but also} \jz{open-source code,}\svp{ initial and any manually processed} \jz{data, documentation detailing language/package versions, random seeds, metadata,} \svp{and other essential details.}
```
...
\svp{While this example is compelling, it is far from unique: }

<!-- In recognition of the additional requirements of full reproducibility,  -->
\jz{some journals have adopted policies encouraging or requiring that authors provide code and data sufficient to reproduce the statistical analyses, with the goal of building a "culture of reproducibility" in their respective fields \citep{peng_reproducible_2009, peng_reproducible_2011, stodden_toward_2013}.
...
\svp{In addition to the benefits to forensic examiners who may use CMC algorithms to support decisions made about evidence, the full reproducibility standard benefits the wider research community. 
