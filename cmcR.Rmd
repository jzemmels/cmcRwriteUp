...
\begin{figure}\label{fig:preprocessing-schematic}
\includegraphics[width=\linewidth]{images/preProcessingDiagram.png}
\caption{preprocessing schematic diagram, showing the different components of the preprocessing algorithm used in the different CMC implementations.}
\end{figure}
...
It is \svp{also} unclear how the dropouts and outliers are "removed" from the scan or how outliers are defined\svp{operationalized? outliers typically have a definition (1.5*IQR from upper or lower quartile), but I somehow doubt that's what they're using...}. 

\jz{XXX Chapter 6 of this PhD thesis 10 different criteria by which outliers can be defined in surface metrology: http://oa.upm.es/57106/1/CHEN_WANG.pdf describes XXX}

\svp{XXX It seems odd that there wouldn't be an open source version if it's an ISO standard... XXX}
...
To our knowledge, there does not exist an open-source implementation of the multivariate bandpass Gaussian regression filter used in surface metrology \citep{ISO16610-71}. 
Further, there are various parameters requiring specification to implement a Gaussian regression filter and it is not clear how these parameters were chosen \citep{brinkman_bodschwinna_2003}.\svp{XXX Make sure this is the most-cited multivariate gaussian filter paper, if possible -- just double check to see what's commonly cited elsewhere XXX}
...
\svp{In order to reproducibly implement the preprocessing procedure, we modified a procedure from \citet{tai_submitted_2019} used to automatically process surface matrices.} \jz{XXX I was mistaken -- the RANSAC method isn't from \citet{tai_fully_2018}, but from Xiao Hui's dissertation (specifically, from her unpublished package \pkg{cartridges3D}). XXX}
...
fadul1.1_ransac$surface.matrix <- fadul1.1_ransac$surface.matrix %>%
  cmcR::preProcess_ransac(iters = 600) %>%
...
fadul1.1_noFP$surface.matrix <- fadul1.1_noFP$surface.matrix %>%
  preProcess_removeFPCircle(aggregation_function = function(x,na.rm){median(x,na.rm) - 11})
...
```{r, echo=FALSE,cache = T,fig.cap='\\label{fig:processingPipeline} Illustration of \\pkg{cmcR} preprocessing pipeline.',fig.align='center',fig.pos='htbp',out.width='1.2\\textwidth'}
cmcR::x3pListPlot(list("(1) Resize Surface Matrix" = fadul1.1_downsampled,
                       "(2) Residuals from RANSAC" = fadul1.1_ransac,
                       "(3) Crop Exterior" = fadul1.1_cropped,
                       "(4) Remove Firing Pin Obs." = fadul1.1_noFP,
                       "(5) Band-pass Filter" = fadul1.1_filtered),
                  type = "faceted",
                  legend.quantiles = c(0,.01,.1,.5,.9,.99,1))
```
\jz{The preprocessing procedures implemented in the \pkg{cmcR} package are as follows. First, if desired, a surface matrix is resized by sampling pixels at a user-defined rate. It is common in the CMC literature to sample every other row and column in a surface matrix, resulting in a matrix that is approximately 25% of original's dimension. This downsampling is performed using the \code{x3p\_sample} function from the \pkg{x3ptools} package (CITE lines num-num in [Appendix](). An example of a resized yet unprocessed surface matrix is shown in the top-left corner of \autoref{fig:processingPipeline}.}

\jz{After downsampling, the height value of the breech face impression region is estimated using the Random Sample Consensus (RANSAC) robust iterative plane-fitting algorithm \citep{ransac}. Then, either observations or residuals within a small, user-defined distance of the fitted plane are extracted from the surface matrix. Extracting residuals as opposed to "raw" observations helps remove the global trend in height values seen in \autoref{fig:processingPipeline} from the top-right to the bottom-left corner. Other than applying a high-pass Gaussian filter to attenuate large-scale structure, it is unclear how or if such trends are dealt with in the CMC literature. The implemented RANSAC and leveling procedure was adapted from the \pkg{cartridges3D} package \citep{cartridges3D} (CITE lines num-um in [Appendix](). The second plot in \autoref{fig:processingPipeline} shows the residuals after applying the RANSAC method. As we can see, the RANSAC leveling procedure has isolated the breech face impression region. However, there are still undesired observations within the firing pin impression circle and in the corners of surface matrix.}

\jz{To remove observations in the corners of the surface matrix, a simple, automatic cropping procedure is applied. The columns consisting of the middle 20% of the surface matrix are extracted from surface matrix. Then, the number of non-\code{NA} pixels in each row is counted and compared to a user-defined cropping threshold. All rows between the first and last rows with a non-\code{NA} pixel count greater than or equal to the cropping threshold are selected from the entire surface matrix. The same is done with the rows consisting of the middle 20% of the surface matrix. The result is a surface matrix of smaller dimension, as illustrated in the third plot of \autoref{fig:processingPipeline}.}

\jz{The Hough transform is a popular shape detection method that can be used to detect the often circular firing pin impression left in cartridge cases [\citet{hough}, \citet{gerules_survey_2013}]. Given a radius, the circular Hough transform can be used to identify the center of a circle in an image. However, firing pin impressions and thus their radii are not uniform across cartridge cases. Thus, the radius must first be estimated before the Hough transform can be applied. To estimate the radius, a novel approach is used. First, the values in the surface matrix are binarized into non-\code{NA} and \code{NA} classifications. Then, the number of non-\code{NA} pixels in each row is counted. The distance between the two largest modes of this count-per-row series is an estimate for the diameter of the firing pin impression circle. The count-per-row series is smoothed using a user-defined moving-average filter (via \code{zoo::rollmean}). The smoothed series is then differenced and the indices at which the differenced values' signs change from positive to negative are determined. Assuming the smoothing filter is adequate, the distance between these indices should estimate the firing pin impression circle diameter well. For added robustness, this procedure is also applied to the count-per-column series and repeated after the surface matrix is rotated by $15, 30, 45, 60$ and $75$ degrees. A total of 12 candidate radius estimates are calculated by this procedure. These are combined via a user-defined aggregation function to determine a consensual radius estimate.}

\jz{The radius estimation procedure will often yield decent estimates of the true firing pin impression circle radius. However, the Hough transform estimated center can be quite sensitive to small changes in the radius value provided; especially if the perimeter of the circle is perforated as is often the case with cartridge cases. Thus, a more robust procedure is required than using a single radius value. The novel procedure used in the \pkg{cmcR} package is as follows. A user-defined grid of radius values centered on the consensual radius estimate is created. The Hough transform method is applied for each radius value in this grid. Since the Hough transform is a consensus-based algorithm, a large number of candidate circles are proposed and each is assigned a normalized score representing how "popular" it is among the pixels in the surface matrix. We take the candidate circle with the highest score for each radius value. This sequence of scores tends to be stable and near 1 for radius values close to the true radius of the firing pin impression circle and volatile otherwise. Thus, we determine the longest consecutive sequence of scores that are above some user-defined threshold and choose the circle associated with the median radius value of this sequence as the final estimate. The surface matrix after filtering pixels within the firing pin impression circle are shown in the fourth plot of \autoref{fig:processingPipeline}.}

\jz{Finally, a band-pass filter is applied to the processed surface matrix to reduce the effects of undesired frequencies in the comparison procedure. For example, low frequency (high wavelength) global structure may exist in a cartridge case scan due to manufacturing specifications. As such, cartridge cases from the same manufacturer may be naturally more similar than those from different manufacturers. While at first this may appear to be a boon for classifying matching cartridge cases, note that not removing such information before making a comparison might increase the chances of making a false-positive identification. The goal in this analysis is to make identifications based on breech face impressions alone, so it is important that the inclusion of any other information be avoided, if possible. The effects of global structure can be attenuated using a high-pass Gaussian filter. Conversely, high frequency (low wavelength) noise and outliers persist in cartidge case scans due to, for example, imperfections in the scanning process. Such observations may also reduce the accuracy of our identifications. Their effects can be mitigated by applying a low-pass Gaussian filter that operates as a local, moving average. A band-pass Gaussian filter combines the effects of a low and high-pass filter. Various procedures exist to implement low, high, and band-pass Gaussian filters. The current implementation for the low-pass filter involves constructing a Gaussian kernel based on a user-specified wavelength to attenuate and convolving the kernel with the surface matrix in the frequency domain \citep{computerVision}. A high-pass filter of a particular wavelength is applied by subtracting-away the complementary low-pass filtered surface matrix. A band-pass filter of two wavelengths is applied by sequentially applying a high-pass of the greater wavelength followed by a low-pass filter of the lesser \citep{gauss_filter_source}.}
...
<!-- This was inspired largely by \citet{tai_fully_2018} who detail a fully-automatic procedure for processing 2D cartridge case images.  -->
...
In order to perform the comparison procedure, dropouts and other \code{NA}-valued pixels must be replaced with a non-\code{NA} value. 
The convention adopted in the \pkg{cmcR} package is to use the average pixel value as a replacement. 
There is currently no determination or removal of outliers in the \pkg{cmcR} package's preprocessing procedures.
\svp{Why? Can you justify this decision?}
\jz{XXX Both how outliers are defined as well as how observations are "removed" from the scan are unclear based on descriptions provided in the CMC literature. \citet{song_3d_2014} is the only paper in the CMC literature that references the removal of outliers and the low-pass part of the Gaussian filter reduces the effects of high frequency noise anyways. XXX}
\svp{XXX Could we embed a scan in the margin showing each subsequent stage of preprocessing? Would that show anything useful? Not sure, but I like visuals.}
...
For computational purposes it is common the CMC literature to down-sample a surface matrix prior to performing the correlation cell comparison procedure. 
The sample_x3p function from the x3ptools package can be used used to sample every m th row/column of a surface matrix. 
The \code{selectBFImpression\_sample\_x3p} function performs all of these preprocessing procedures in a single call. 
The code to produce the first surface matrix shown in \autoref{fig:processedScans} is given below while the code for the second surface matrix is skipped for redundancy.
...
If a large number of these correlation cells are deemed highly similar to regions in the other surface matrix, then there is evidence that the two cartridges cases match. 
Thus, the number of highly similar cells can be used as a more granular similarity metric.
...
The scan on the left is divided into a grid of $8 \times 8$ cells. 
...
A variety of interpolation schemes exist and it is unclear how this is performed in the CMC literature \citep{parker_comparison_1983}.
...
The index at which the CCF attains a maximum represents the translations needed to align $B$ with $A$.

It is unclear in the CMC literature whether the CCF between a cell/region pair is calculated from the definition or, as is more commonly done in image processing \citep{Brown92asurvey}, using a result known as the Cross-Correlation Theorem. 
For two matrices $A$ and $B$, The Cross-Correlation Theorem implies that
$$
(A \star B )[m,n]= \mathcal{F}^{-1}\left(\overline{\mathcal{F}(A)} \cdot \mathcal{F}(B)\right)[m,n]
$$
where $\mathcal{F}$ and $\mathcal{F}^{-1}$ denote the discrete Fourier and inverse discrete Fourier transforms, respectively, and the overline denotes the complex conjugate \citep{fft_brigham}. 
...
Surfaces matrices are rotated using a "nearest-neighbor" interpolation scheme \citep{imager}.
