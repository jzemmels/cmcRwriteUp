...
The process by which an examiner arrives at their evidentiary conclusion is largely opaque and has been criticized \citepalias{pcast} \svp{because its' subjectivity does} not allow for an estimation of error rates.
In response, \citet{council_strengthening_2009} pushed to \svp{augment} subjective decisions made by forensic examiners with automatic algorithms that objectively assess evidence and can be explained during court testimony.
\svp{In addition to the objectivity of these algorithms, there is an additional benefit: we expect that an algorithm run on the same data multiple times will produce the same answer; that is, that the results are repeatable. This is extremely beneficial because it allows the prosecution and defense to come to the same conclusion given objective evidence or data.
}

## Repeatability and Reproducibility 
\svp{Repeatability in forensic labs is enforced primarily using standard operating procedures (SOPs), which specify the steps taken for any given evaluation, along with the concentrations of any chemicals used, the range of acceptable machine settings, and any calibration procedures required to be completed before the evidence is evaluated.}
\svp{When labs use computational procedures, this SOP is augmented with specific algorithms, which are themselves SOPs intended for use by man and machine. 
...
\svp{This form of reproducibility requires that the input data, code, method, and computational environment are all described and made available to the community. 
<!--I'm actually fudging  a bit here, since it doesn't require that the results are repeatable by others... -->
...
\svp{Instead, } we find it useful to consider a more inclusive hierarchy of reproducibility.
...
\end{description}
\end{definition}
\jz{To aid in comprehension of an algorithm, it is useful to supplement conceptual descriptions with pseudocode. However, a conceptual description and pseudocode alone does not contain sufficient detail (e.g., parameter settings) to ensure computational reproducibility.
...
<!-- In high-stakes fields like forensics, where it is necessary that a method's strengths and weaknesses are well-understood by practitioners, \hh{ the legal and forensic communities}\svp{, and} \hh{, ideally, the public at-large}, open-source algorithms provide a transparency that manufacturers of proprietary algorithms can not or will not provide. -->
...
<!-- \svp{xxx use of such? -- phrasing} In the absence of a general consensus, it is plausible that the prosecution and defense could each choose a set of parameters and arrive at conflicting results. -->
...
<!-- \svp{This doesn't flow that well because we used a different form of the word reproducibility... fix? rephrase?}  -->
<!-- \jz{Following the \citet{nas_2019}, we define \dfn{reproducibility} to mean computational reproducibility: "obtaining consistent computational results using the same input data, computational steps, methods, code, and conditions of analysis."} -->
...
<!-- \jz{While a reproduction of results by the original research group is the bare minimum to achieve scientific rigor, it is not particularly helpful in consequential fields like forensic science where results need to be precisely reproducible by forensic practitioners and other research groups.}  -->
<!-- In computational research, reproducibility of results is commonly used to mean that the code and data used to produce published results be made publicly available \citep{peng_reproducible_2011}.  -->
...
<!-- <!-- \svp{We may need to rephrse this -- we're saying "in computational research, reproducibility means..." and then using the phrase "computational reproducibility" to mean something different}  --> 
<!-- In this light, computational reproducibility in forensic science is all the more salient because it enables discussions about algorithmic transparency and accountability \citep{kwongAlgorithmSaysYou2017, desaiTrustVerifyGuide2017}. -->
<!-- \jz{This sentence sounds better in the conclusion, I think:} --> 
<!-- Therefore, it is of the utmost importance \svp{too many uses of important} that the forensic community does not go only halfway, trading a subjective, human black box for objective, proprietary algorithms that are similarly opaque and unauditable. --> 
<!-- As a community, we need to develop our algorithms from the ground up with the intention that any results be immediately computationally reproducible by \jz{interested parties, such as another research group,} rather than eventually qualitatively reproducible. \svp{you haven't introduced the by whom question, so this is a bit disjoint} -->
...
<!-- In this paper, we focus on cartridge case comparison algorithms. -->
...
The Congruent Matching Cells (CMC) \jz{pipeline is a collection of algorithms developed at the National Institute of Standards and Technology (NIST) in 2012 to process and compare cartridge case evidence \citep{song_proposed_2013}.} Since then, the pipeline and its numerous extensions \citep{tong_improved_2015,chen_convergence_2017,song_estimating_2018} have shown promise in being able to differentiate between matching and non-matching cartridge cases.
...
\svp{In the remainder of this paper, we describe}
<!-- We then follow with a description of the implementation of -->
a general, \svp{reproducible, and open-source} CMC pipeline which encompasses \jz{those} discussed in \citet{song_proposed_2013}, \citet{song_3d_2014}, and \citet{tong_improved_2015}.
...
\jz{Same-sourceness can only be determined by matching individual characteristics, so the global structure is removed by using a high-pass Gaussian filter.} Similarly, a low-pass filter is used to remove noise and outliers from cartridge case scans due to, for example, imperfections in the scanning process.
...
While it is unclear how the CCF is implemented in the CMC papers <!-- (it is not even defined mathematically) --> , the \CRANpkg{cmcR} package adopts the following conventions:
...
In particular, the \code{decision\_CMC} function applies both the \jz{original and High CMC decision rules} depending on whether a value for $\tau$ is provided.
...
We have demonstrated that the three common reasons for unreproducible results that are discussed in the introduction: ambiguity in how procedures are implemented, missing or incomplete data, and missing or incomplete code, are all applicable to the Congruent Matching Cells pipeline.
Our open-source implementation provided in \CRANpkg{cmcR}, paired with the open-source cartridge case data available in the NIST Ballistics Toolmark Research Database \citep{nbtrd}, avoids these problems and encourages validation and improvement from the wider forensics community.}
```

\jz{\textbf{Paragraph 1: Refer back to hierarchy of algorithmic transparency with additional discussion}}

```{=tex}
\jz{\textbf{Review the hierarhcy of algorithmic transparency and re-assert the need for computational reproducibility.}}

\jz{At the moment, we have a conceptual description of the CMC pipeline.
This conceptual description is undoubtedly important as it can be difficult to understand the pipeline solely by reading through its source code.
However, the conceptual description provides only enough detail to qualitatively reproduce the original results.
To ensure forensic transparency, the wider forensics community needs to be able to computationally reproduce the original results.
As we have seen, we have neither the original source code nor the manually-processed input data to do so.}

\jz{\textbf{Describe factors that cause unreproducibile results if only conceptual descriptions or pseudocode are provided.}}

\jz{When translating an algorithm from a conceptual description or pseudocode, reproducibility of results is challenging as an implementation may depend on the creator's programming habits or the programming language used.
...
\jz{\textbf{Conclude that, in-general, open-source and open data is the best way to ensure algorithmic transparency}}

\jz{Open-source algorithms and data, accompanied with documentation detailing language/package versions, random seeds, metadata, etc., most clearly ensures computationally reproducible results in this hierarchy.}
```

<!-- \jz{XXX I don't really know where to put this next paragraph and table. Susan: were you thinking that would fit well in the intro or in the discussion? XXX} -->
...
\jz{\textbf{Paragraph 2: Discuss how the manner in which the pipeline is currently presented doesn't lead to community-wide acceptance.}}

```{=tex}
\jz{The Daubert Standard requires that a scientific method be widely accepted by a relevant scientific community before it can be used in an expert witness's testimony.
Prior to acceptance, the relevant scientific community must validate a method, which includes a careful study of the method's sensitivites to different processing choices.
In computational research, a lack of computational reproducibilty makes it impossible to perform such an analysis.}

\jz{\textbf{In particular, point to the sensitivity of the method to various parameter choices.}}

\jz{For example, we have highlighted a sensitivity of the CMC pipeline to parameter choice that hasn't been discussed prior to our implementation.
The broad graphical or numerical summaries currently used to summarize the performance of the CMC pipeline, while admissible from a readability perspective, do not allow us to contrast different algorithms or parameter choices at the individual comparison level.}

\jz{\textbf{Conclude that the forensic science community should adopt a "culture of reproducibility" by using open-source platforms}}

\jz{In recognition of the additional requirements of computational reproducibility, some journals have adopted policies encouraging or requiring that authors provide code and data sufficient to reproduce the statistical analyses, with the goal of building a "culture of reproducibility" in their respective fields \citep{peng_reproducible_2009, peng_reproducible_2011, stodden_toward_2013}.
...
We argue that these standards should be adopted by the forensic science community by leveraging open-source ecosystems like the R and GitHub communities.}
```

\jz{\textbf{Paragraph 3: Describe how our implementation can be used as a foundation for further improvements upon the CMC method and how the package overall can be used as a template.}}

```{=tex}
\jz{The modular approach used in \CRANpkg{cmcR} makes it easier to describe the algorithm, easier to write the code, modify and improve the method, and show that the improvement has a noticeable impact on the results. 
...
<!-- \jz{XXX I think some of the ideas in the following paragraph should go into the conclusion instead XXX} -->
