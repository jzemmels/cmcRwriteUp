...
abstract: > 
  A tenet of scientific research is the ability to use the same procedures and material as a previous study to reproduce the same results or conclusions. As the need for computationally-intensive methods to analyze large amount of data grows, the criteria needed to achieve reproducibility, specifically computational reproducibility, have become more sophisticated. In general, prosaic descriptions of algorithms, however precise, are not sufficient to ensure that a method will be completely reproducible. Results may be sensitive to conditions not commonly specified in written-word descriptions such as implicit parameter settings or the programming language used. To achieve true computational reproducibility, it is necessary to provide all intermediate data and code used to produce published results. In this paper, we consider a class of algorithms developed at the National Institute of Standards and Technology (NIST) to perform firearm evidence identification on cartridge case evidence known as the \dfn{Congruent Matching Cells} (CMC) methods. To date, only prosaic descriptions of these algorithms have been published. We introduce the first open-source implementation of the Congruent Matching Cells methods in the R package \pkg{cmcR} and use it to illustrate problems that can arise when only computationally ambiguous descriptions of algorithms are provided.
...
\jz{T}he source code to the full `cmcR` package is accessible at https://github.com/CSAFE-ISU/cmcR.
}
...
<!-- I've tried to make this a bit more concise -- so that people don't lose the point while reading this section -->
\svp{All of the CMC methods can be broken down into three primary steps: preprocessing, cell-based similarity feature extraction, and application of a decision rule. 
The primary difference between the original method of \citet{song_proposed_2013} and the high CMC method is the decision rules utilized to separate matches and non-matches, but there are also several small differences in the preprocessing and comparison procedures.} 
<!-- While numerous CMC-related methods have been proposed since the original \citet{song_proposed_2013} paper, all of these methods can be divided into three sub-procedures.  -->
...
Such a discussion based on our implementation of the \jz{origina method of \citet{song_proposed_2013}} and the High CMC method can be found [below](investigation)}.
<!-- \svp{XXX found where now? XXX} -->

\svp{In this section, we will discuss e}\jz{ach sub-procedure of the CMC method}
<!-- will be described  -->
...
\svp{Then, in \autoref{implementation}, we will examine the implementation of each sub-procedure in the \pkg{cmcR} package, considering the differences between the text description and \jz{its algorithmic translation}.}
...
\jz{The decision rule} \citet{song_3d_2014} propose using \jz{is based on}
...
The High CMC method (as it would come to be called) was proposed by \citet{tong_improved_2015} to address situations in which the \jz{original method of \citet{song_proposed_2013}} is excessively strict. 
The proposed extension to the \jz{original method of \citet{song_proposed_2013}} of utilizes the behavior of the CCF$_{\max}$ values across various rotations more advantageously. 
...
As mentioned above, such pairs would be assigned the minimum of the two CMC counts determined in both directions under the original method of \citet{song_proposed_2013}.
...
Additionally, we might require the $\theta_{\text{ref}}$ value determined under the original method of \citet{song_proposed_2013} to agree with the modal $\theta$ value determined under the High CMC method, say $\theta_{\text{mode}}$.
Since the original method of \citet{song_proposed_2013} and High CMC criteria are applied in both directions for the High CMC method, we can determine specifically if $|\theta_{\text{ref},1-2} - \theta_{\text{mode},1-2}| \leq T_\theta$ and $|\theta_{\text{ref},2-1} - \theta_{\text{mode},2-1}| \leq T_\theta$ where the subscripts emphasize the direction considered. 
If either of these inequalities fail to hold, then a cartridge case pair is assigned the minimum of the two CMC counts determined in both directions under the original method of \citet{song_proposed_2013}.}

# Implementation in the \pkg{cmcR} package {#implementation}
...
```{r,echo=FALSE,cache = T,fig.cap='\\label{fig:processedScans} Fadul 1-1 and Fadul 1-2 after preprocessing. Similar striated markings are now easier to visually identify on both surfaces.',fig.align='center',fig.pos='htbp',out.width='\\textwidth'}
...
In this section, we will investigate how ambiguities identified in the CMC method descriptions (\autoref{sec:cmcMethod}) induces a high degree of sensitivity to choosing the "correct" processing conditions.
...
While far from exhaustive, we focus this discussion on what we have found to be highly influential factors on the methods' sensitivity.
These factors include sensitivity to the choice of congruency thresholds (discussed in the [section describing the orignal method's decision rule](\#originalMethod)), to the reader's interpretation of the methods' descriptions, and to the preprocessing procedures employed before feature-extraction.

## Sensitivity to experimentally-determined congruency thresholds
...
```{r include=FALSE,eval=FALSE,fig.cap='\\label{fig:topVoteCMC_perfectIdentification} CMC count distribution under the original method of \\citet{song_proposed_2013} with $T_{\\text{CCF}} = .6$, $T_{\\Delta x} = T_{\\Delta y} = 20$, and $T_{\\theta} = 6$ degrees.',fig.align='center',fig.pos='htbp',out.width='\\textwidth',out.height=".55\\textwidth"}

barHeights <- cmcs_downSampled %>%
  select(originalMethodCMCs,type,pairName,ccf_thresh,dx_thresh,theta_thresh) %>%
  distinct() %>%
  ...
  group_by(type,originalMethodCMCs) %>%
  tally() %>%
  ungroup() %>%
  group_by(type) %>%
  mutate(prop = n/sum(n))
...
cmcs_downSampled %>%
  ...
  rename(`CCF Threshold` = ccf_thresh,
         `Trans. Threshold` = dx_thresh) %>%
  ggplot() +
  stat_density(aes(x = originalMethodCMCs,y = ..density.., fill = type)) +
  geom_bar(aes(x = originalMethodCMCs,fill = type)) +
  # facet_wrap(~`Trans. Threshold`,labeller = label_both) +
  geom_text(data = barHeights,
            aes(x = originalMethodCMCs,y = n,label = n),
            nudge_y = 7,size = 2.5) +
  scale_fill_manual(values = c("#a60b00","#1b03a3"),
...
\jz{\autoref{fig:topVoteCMC_sensitivity} illustrates the sensitivity of the original method of \citet{song_proposed_2013} to various $T_{CCF}$ and $T_{\Delta x}, T_{\Delta y}$ values where $T_{\theta} = 6$ degrees. 
...
## Multiple interpretations of prosaic descriptions
...
## Ambiguous descriptions of preprocessing procedures
