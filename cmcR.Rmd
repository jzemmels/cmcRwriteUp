...
That is,--> \hh{propose the median as} a consensus <!--is determined by finding the median registration phase values,--> across the cell/region pairs for a particular cartridge case pair comparison.
Then, the distances between the consensus registration values and the cell comparison values are assessed to determine whether they are within a specified distance of the consensus, as determined by three threshold parameters $T_{x}, T_{y}, T_\theta, T_{\text{CCF}}$.
...
The High CMC utilizes the behavior of the CCF$_{\max}$ values across various rotations more \jz{carefully} \hh{XXX don't agree with that either. Can we leave out the judgment?} than the original method of \citet{song_proposed_2013}. 
...
\jz{XXX Yes, that is what actually happens in the High CMC method. \hh{XXX that is not how I read the above sentence. The above sentence suggests that the original method uses the minimum.} We have no idea if the comparisons are performed in both directions for the original method. \citet{tong_improved_2015} was the first to explicitly mention peforming comparisons in both directions.}
