...
\svp{In this paper, we describe the implementation of a general CMC method which encompassesses methods described in \citet{song_proposed_2013}, \citet{song_3d_2014}, \citet{tong_improved_2015}, \citet{chen_convergence_2017}, and \citet{tong_valid_2018}. \citet{song_proposed_2013} lays out the conceptual framework for the original CMC method, which is referred to as the ``Top Vote" method in subsequent literature; a subsequent improvement of the method presented in \citet{tong_improved_2015} and used in subsequent papers is referred to as the ``High CMC" method. Both implementations (and subsequent proposed improvements) can be used to correctly differentiate same and different-source cartridge cases from the test dataset \citep{fadul_empirical_nodate}.} 
...
Such a discussion based on our implementation of the Top Vote and High CMC methods can be found in the REF.} \svp{XXX found where now? XXX}
...
\svp{Then, in \autoref{implementation}, we will examine the implementation of each sub-procedure in the \pkg{cmcR} package, considering the differences between the text description and actual code. XXX I don't like the use of actual here, but can't think of another way to get the point across... any ideas? XXX}
