...
\caption{Procedures by which similarity features are obtained for two cartridge case scans differ across CMC papers.}\label{fig:cmc-schematic}
\end{figure}
...
\svp{\autoref{fig:cmcflow-schematic} shows the steps of the correlation cell comparison process in each of the papers as well as the \pkg{cmcR} implementation.}

\begin{figure}
\includegraphics[width=\linewidth]{images/cmc_flow.png}
\caption{Each CMC implementation uses slightly different procedures. Steps which are implemented with additional user-specified parameters are shaded purple; steps which are described but without sufficient detail are shaded grey.} \label{fig:cmcflow-schematic}
\end{figure}
...
\jz{XXX I removed the word "two" from "describes two slightly different methods" because each paper in the CMC literature after the high CMC paper describe slight variations on the base method. Our goal with this project was to implement the high CMC method, which required also implementing the top vote method. XXX}
...
\citet{song_3d_2014} proposes using 
...
\jz{\autoref{fig:translationVotes} shows the distributions of the $(x_i, y_i)$ values which produce the highest CCF for the alignment of each cell/region pair of a known match and known non-match comparison. The squares in \autoref{fig:translationVotes} are centered on the comparisons' respective $(x_{\text{ref}},y_{\text{ref}})$ values and represent the maximum distance an $(x_i,y_i)$ estimated translation value can be from $(x_{\text{ref}},y_{\text{ref}})$ to be classified as "congruent" based on thresholds $T_{x} = T_{y} = 20$ pixels. Of the 43 cell/region pairs considered in the known match comparison, 28 $(x_i,y_i)$ values are within the specified thresholds of $(x_{\text{ref}},y_{\text{ref}})$. In contrast, 14 out of 42 $(x_i,y_i)$ are within the specified thresholds of $(x_{\text{ref}},y_{\text{ref}})$ for the known non-match comparison.}
...
```{r echo = FALSE,cache = T,fig.align = 'left',fig.cap='\\label{fig:translationVotes} Estimated translation values within 15 pixels of median value for 1 Known Match and 1 Known Non-Match cartridge case pair',fig.pos = 'h'}
bind_rows(kmComparison$comparison_1to2$ccfResults %>%
            topResultsPerCell() %>%
            ungroup() %>%
            mutate(`Comparison Type` = "Known Match"),
          knmComparison$comparison_1to2$ccfResults %>%
            topResultsPerCell() %>%
            ungroup() %>%
...
\jz{\autoref{fig:rotationVotes} shows the distribution of the $\theta_i$ values which produce the highest CCF for the alignment of each cell/region pair of the known match and known non-match comparison. A vertical line is drawn at the comparisons' respective $\theta_{\text{ref}}$ value and a threshold of $T_{\theta} = 6$ degrees is used to classify $\theta_i$ values as "congruent." 24 of 43 known match and 4 of 42 known non-match $\theta_i$ values are classified as congruent under these conditions.}

```{r echo = FALSE,cache = T,fig.align = 'left',fig.cap='\\label{fig:rotationVotes} Estimated rotation values within 3 degrees of median value for 1 Known Match and 1 Known Non-Match cartridge case pair',fig.pos = 'h'}
bind_rows(kmComparison$comparison_1to2$ccfResults %>%
            topResultsPerCell() %>%
            ungroup() %>%
            mutate(`Comparison Type` = "Known Match"),
