...
While some researchers in the forensic science community have adopted computationally reproducible habits [cite CSAFE stuff], many of the algorithms described in the literature are not reproducible or open-source.
...
Our package, \pkg{cmcR}, provides an open-source implementation of the CMC method, but it also serves as an example of the implementation barriers which occur when translating textual descriptions of algorithms into detailed source code. 
This process offers some insight into the necessary components which must be available for computational reproducibility. 
Our experience shows that when faced with ambiguity in how a method is implemented, there are only few options available to us apart from performing a brute-force search through combinations of decisions that may have been made to yield the results reported. 
...
#actually the National Ballistics Toolmark Research Database (so their URL
#is mistaken)
...
These are the images used as input to automated comparison algorithms, such as the CMC method originally proposed in \citet{song_proposed_2013}.
...
A subsequent improvement of the method presented in \citet{tong_improved_2015} and used in subsequent papers is referred to as the ``High CMC" method \citep{chen_convergence_2017}. Both implementations (and subsequent proposed improvements) can be used to correctly differentiate same and different-source cartridge cases from the dataset \citep{fadul_empirical_nodate}.

The \pkg{cmcR} package contains implementations designed for use with 3D topographical scans of the original method described in \citet{song_proposed_2013} and \citet{song_3d_2014} and the High CMC method described in \citet{tong_improved_2015}.
...
All of the CMC methods can be broken down into three broad stages: preprocessing, cell-based similarity feature extraction, and application of a decision rule as illustrated in \autoref{fig:overview-flow}. \hh{In the following sections we break each of these stages further into a set of modular steps.}
\hh{One advantage of modularizing} these algorithms is that we can implement an algorithm as a  set of \hh{sequential} procedures. \hh{This allows us to} then test new variations against the old \hh{implementation} in a coherent, unified framework.
...
Current CMC literature lacks justification for changing these procedures. However, different conditions yields considerably different results while applying the same method. 
This sensitivity of various methods to different conditions has also not been discussed in CMC literature.


In this section, we discuss each stage of the CMC method
using excerpts from the original papers.
...
Cartridge case scans are commonly stored in the ISO standard x3p file format \citep{ISO25178-72}. An x3p consists of a single surface matrix representing the height value of the breech face surface at a particular location and metadata concerning the parameters under which the scan was taken (size, resolution, creator, microscope, and microscopy software versions etc.). The \pkg{x3ptools} package \citep{x3ptools} can be used to work with these x3p files in R \citep{x3ptools}.
...
Complicating the issue \hh{of preprocessing the scans}, the procedures used to process the surface matrices differ in each paper in the CMC literature. \citet{song_3d_2014} outline the following preprocessing procedure:
...
\jz{The preprocessing procedures are implemented via the modularized functions of the form \code{preProcess\_*}.
...
\jz{We demonstrate usage of the \code{preProcess\_*} functions on the Fadul 1-1 scan. 
Following each code chunk is an explanation of the functions used.}
...
\jz{After reading in the breech face scan, three major regions of the scan are identified via a labeling algorithm described in \citet{hesselink_concurrent_2001} and available in the \pkg{imager} package \citep{imager}. 
...
We then remove any large-scale trend that exists in the breech face scan height values. In \jz{steps (1) and (2) of \autoref{fig:processingPipeline}} \svp{it is clear that there is a} \jz{southwest-to-northeast trend in height values. 
...
\svp{It is unclear how such trends in height values are dealt with in the CMC literature; certainly, a high-pass Gaussian filter will remove some structure, but it is not explicitly mentioned whether any leveling is performed manually as well.} The \code{preProcess\_removeTrend} functions levels the breech face impression regions obtained from step (2). The function estimates and subtracts the conditional median via the \code{rq} function of the \pkg{quantreg} package \citep{quantreg}. Step (3) of \autoref{fig:processingPipeline} shows a median-leveled breech face scan.
...
\svp{In the final preprocessing step,} a band-pass filter is applied to the processed breech face scan to reduce the effects of undesired frequencies in the comparison procedure. 
...
The goal of this analysis is to match scans based on individualizing characteristics, and so we remove the global structure using a high-pass Gaussian filter. Similarly,} high frequency (low wavelength) noise and outliers persist in cartridge case scans due to, for example, imperfections in the scanning process. 
...
The band-pass filtered breech face scan as returned by the \code{preProcess\_gaussFilter} function is shown in the fifth panel of \autoref{fig:processingPipeline}.
...
\jz{\autoref{fig:processedScans} displays the processed Fadul 1-1 and Fadul 1-2 scans}; \svp{the second matrix is processed using the same function arguments}. \jz{Similarity features are extracted from a process cartridge case pair in the cell-based comparison procedure.}
...
Instead, \citet{song_proposed_2013} proposes partitioning one breech face scan into a grid of "correlation cells," some of which will enclose the valid correlation regions of interest. 
If a large \svp{proportion} of these correlation cells are deemed highly similar to regions in the other breech face scan, then there is evidence that the two cartridges cases match. \citet{song_proposed_2013} propose the
the number of highly similar cells, the \dfn{CMC count}, as a more \svp{robust} similarity metric, \svp{compared to a score derived from the entire cartridge case}.
...
\jz{The steps of the cell-based comparison procedure are implemented via the \code{comparison\_*} functions.
...
\svp{As shown above, this function is applied to each $\theta$ value in sequence, rotating the target scan relative to the reference scan. When implementing the High CMC method, both combinations of reference and target scan are examined (e.g. A-B and B-A).}

\autoref{tab:cellCCF} shows several rows of the data frame output of the \code{comparison\_allTogether} function.
Although a grid of $8 \times 8$ cells were used, there were between 27 cell/region pairs that contained a sufficient proportion of non-missing values (15% in this example). 
...
In this section, we describe two particular \jz{decision rules} that are implemented in the \pkg{cmcR} package. These are the decision rules used in the original method of \citet{song_proposed_2013} and the High CMC of \citet{tong_improved_2015}.

### The original method of \citet{song_proposed_2013} {#originalMethod}

\jz{This section briefly describes the decision rule of the original method of \citet{song_proposed_2013}. For a thorough explanation of the procedure, refer to [the vignette] in the \pkg{cmcR} package.}
...
\jz{This section summarizes how the High CMC method differs from the original method of \citet{song_proposed_2013}. We also point out ambiguities in the textual description of the High CMC method that renders the method impossible to reproduce. For a thorough discussion of the High CMC method refer to [the vignette] in the \pkg{cmcR} package.}
...
\citet{tong_improved_2015} introduce an additional criterion to identify a mode in the CMC-$\theta$ distribution. 
...
Based on this procedure, \citet{tong_improved_2015} outline the following steps of the High CMC method:
...
Recall from} [High CMC method](#highCMCMethod) \jz{section that the High CMC criteria attempts to identify a "prominent peak" or mode in the CMC-$\theta$ distribution.
...
Let $\theta_{\text{mode},1-2}$ and $\theta_{\text{mode},2-1}$ be the modal $\theta$ values identified in the two comparison directions (note that either or both of these may be missing values if the associated direction fails the High CMC criteria).
...
In this case, the cartridge case pair would "fail" the High CMC criteria and be assigned the minimum of the two CMC counts determined in both directions under the original method of \citet{song_proposed_2013}.
A more liberal alternative would be to behave as if both directions pass the High CMC criteria if only one direction passes.
...
Let CMC$_{i,j_i}$ denote the CMC count assigned to the $j_i$th cartridge case pair, $j_i = 1,...,n_i$, $n_1 = 63, n_2 = 717$, from the $i$th group, $i = 1,2$ representing matches and non-matches, respectively. 
...
In particular, we treat a failure of the High CMC criteria in either direction as a failure in both directions.
We also enforce agreement between the $\theta_{\text{ref},1-2}, \theta_{\text{ref},2-1}$ from the original method of \citet{song_proposed_2013} and the High CMC $\theta_{\text{mode},1-2}, \theta_{\text{mode},2-1}$ values by setting \code{compareThetas = TRUE} in the \code{decision\_combineCMCDirections} function. 
...
```{r,echo=FALSE, fig.cap='\\label{fig:highCMC_sensitivity} High CMC count relative frequencies for various $T_{\\Delta x}, T_{\\Delta y}$, and $T_{\\text{CCF}}$ values and $T_{\\theta} = 6$ degrees, \\code{missingTheta\\_decision = "fail"}, and \\code{compareInitialAndHighThetas = TRUE}. Thresholds become more stringent as the $T_{\\Delta x}, T_{\\Delta y}$ values decrease and $T_{\\text{CCF}}$ values increase. AUCs based on the minimum CMC count classification threshold and Between/Within group variance ratios are shown for each combination. The High CMC method tends to yield a larger CMC count than the original method of \\citet{song_proposed_2013} to cartridge case pairs that pass the High CMC criteria. However, this holds for both matches and non-matches that pass the High CMC criteria indicating that the High CMC method is highly dependent on choosing parameters such that no non-match cartridge case pairs pass the High CMC criteria.',fig.align='left',fig.pos='htbp',out.width='\\textwidth'}
...
We consider 5 parameter settings that, based on our experimentation, have a demonstrable impact on the effectiveness of the CMC method. These include:
\begin{itemize}
...
\end{itemize}
To understand the generalizability of a particular set of parameters to "out-of-sample" data, choosing parameter settings resulting in an AUC equal to 1 (perfect identification) is not enough. Instead, we use the variance ratio to identify an optimal set of thresholds. \autoref{fig:cmc_sensitivityScatter} shows the value of the variance ratio under various settings of these 5 parameters. We can see that using the High CMC method on breech face scans for which the trend was removed yields better variance ratio values. Conditional on these settings, choosing thresholds $T_x = 10 = T_y$, $T_\theta = 3$, and $T_{\text{CCF}} \in [.35,5]$ yields the largest variance ratios. These are similar thresholds to those chosen in other CMC papers.

```{r ,echo=FALSE, fig.cap='\\label{fig:cmc_sensitivityScatter} We desire parameters that yield a high variance ratio. Parameters are currently determined by experimentation in the CMC literature and differ between papers. These scatterplots show the behavior of the variance ratio under various parameter settings. Based on the variance ratio statistic, the optimal set of parameter settings include using the High CMC method applied to de-trended breech face scans using translation thresholds $T_{x} = 10 = T_y$, rotation threshold $T_{\\theta} = 3$, and CCF threshold $T_{CCF} \\in [.35,.5]$. Removal of the trend during preprocessing, which is not explicitly discussed in the CMC literature, has a major impact on the effectiveness of the CMC method.',fig.align='left',fig.pos='htbp',out.width='\\textwidth'}

cmcs_aucsDistinct <- cmcs_aucs %>%
  select(ccf_thresh,dx_thresh,theta_thresh,missingTheta_decision,compareInitialAndHighThetas,topVoteCMC_AUC,highCMC_AUC) %>%
  distinct() %>%
  rename(originalMethodCMCs = topVoteCMC_AUC,
         highCMCs = highCMC_AUC) %>%
...
cmcs_noLeveling_aucsDistinct <- cmcs_noLeveling_aucs %>%
  select(ccf_thresh,dx_thresh,theta_thresh,missingTheta_decision,compareInitialAndHighThetas,topVoteCMC_AUC,highCMC_AUC) %>%
  distinct() %>%
  rename(originalMethodCMCs = topVoteCMC_AUC,
         highCMCs = highCMC_AUC) %>%
...
pcpData_leveling <- cmcs_varRatios %>%
  select(ccf_thresh,dx_thresh,theta_thresh,missingTheta_decision,compareInitialAndHighThetas,cmcType,varRatio)  %>%
  left_join(cmcs_aucsDistinct,by = c("ccf_thresh","dx_thresh","theta_thresh","missingTheta_decision","compareInitialAndHighThetas","cmcType"))  %>%
  distinct() %>%
  mutate(cmcType = ifelse(cmcType == "highCMCs","High CMC","Original")) %>%
  mutate(
    # ccf_thresh = factor(ccf_thresh),
    # dx_thresh = factor(dx_thresh,),
    # theta_thresh = factor(theta_thresh),
    missingTheta_decision = factor(missingTheta_decision),
    compareInitialAndHighThetas = factor(compareInitialAndHighThetas),
    cmcType = factor(cmcType)
  )  %>%
  distinct() %>%
  mutate(trendRemoved = TRUE)  

pcpData_noLeveling <- cmcs_noLeveling_varRatios %>%
  select(ccf_thresh,dx_thresh,theta_thresh,missingTheta_decision,compareInitialAndHighThetas,cmcType,varRatio)  %>%
  left_join(cmcs_noLeveling_aucsDistinct,by = c("ccf_thresh","dx_thresh","theta_thresh","missingTheta_decision","compareInitialAndHighThetas","cmcType"))  %>%
  distinct() %>%
  mutate(cmcType = ifelse(cmcType == "highCMCs","High CMC","Original")) %>%
  mutate(
    # ccf_thresh = factor(ccf_thresh),
  ...
  distinct() %>%
  mutate(trendRemoved = FALSE)

# bind_rows(pcpData_leveling,
#           pcpData_noLeveling) %>%
#   mutate(trendRemoved = factor(trendRemoved)) %>%
#   filter(missingTheta_decision == "fail" & compareInitialAndHighThetas == TRUE) %>%
#   rename(`Decision Rule` = cmcType,
#          `CCF Thresh.` = ccf_thresh,
#          `Trans. Thresh.` = dx_thresh,
#          `Theta Thresh.` = theta_thresh,
#          `Trend Removed` = trendRemoved) %>%
#   ggplot(aes(vars = vars(c("Decision Rule","Trans. Thresh.","Theta Thresh.","Trend Removed")))) +
#   ggpcp::geom_pcp(aes(colour = varRatio,
#                       alpha = AUC),
#                   boxwidth = .1) +
#   #This isn't working for some reason:
#   # scale_x_discrete(labels = c("Decision Rule","Trans. Thresh","Theta Thresh","Trend Removed")) +
#   ggpcp::geom_pcp_box(boxwidth=0.1,fill = NA) +
#   geom_pcp_text(boxwidth=0.1) +
#   scale_colour_gradient(low = "#40B0A6",
#                         high = "#E1BE6A") +
#   theme_bw() +
#   guides(alpha = guide_legend(title = "AUC",reverse = TRUE),
#          colour = guide_colorbar(title = "Var. Ratio")) +
...
bind_rows(pcpData_leveling,
  ...
  #        `Theta Thresh.` = theta_thresh,
  #        `Trend Removed` = trendRemoved) %>%
  mutate(`ccf*trans` = as.numeric(ccf_thresh)*as.numeric(dx_thresh),
         perfectIdent = ifelse(AUC == 1,TRUE,FALSE)) %>%
  # filter(cmcType == "High CMC") %>%
  ggplot(aes(x = dx_thresh,
             y = varRatio,
             colour = ccf_thresh)) +
  geom_point() +
  ...
  facet_grid(theta_thresh ~ cmcType + trendRemoved,
             scales = "free",
             labeller = labeller(cmcType = c("High CMC","Original Method") %>% set_names(c("High CMC","Original")),
                                 theta_thresh = c("Theta Thresh.: 3","Theta Thresh.: 6") %>% setNames(c(3,6)),
...
There are other connections that we can draw from \autoref{fig:cmc_sensitivityScatter} and what we have discussed previously as patterns in the CMC literature that make reproducibility impossible. 
In particular, \autoref{fig:cmc_sensitivityScatter} illustrates both the dangers of providing a written-word description of an algorithm and the benefits of breaking a method up into modularized steps to uncover flaws or sensitivities.
We will expand upon these two points below.

## The dangers of prosaic algorithm descriptions

\jz{It should also be noted that the success of the High CMC method hinges largely on selecting processing conditions such that no non-match pairs pass the High CMC criteria. 
That is, the High CMC criteria functions as a rough match vs. non-match decision rule prior to assigning CMCs. 
...
Namely, how cartridge case pairs for which only one comparison direction passes the High CMC criteria should be dealt with.
The \code{missingTheta\_decision} argument of the \code{cmcFilter\_improved} function controls how CMCs are assigned if only one direction passes the High CMC criteria. 
...
\autoref{fig:cmc_sensitivityScatter} demonstrates how results change under 4 combinations of these two arguments with $T_{\Delta x} = T_{\Delta y} = 20$, $T_{\text{CCF}} = .5$, and $T_{\theta} = 6$ degrees.
In particular, considering \code{missingTheta\_decision = "replace"} vs. \code{"fail"}, which treats passing the High CMC criteria in one direction as a pass or fail in both, respectively. 
...
These large false-positive CMC counts are associated with cartridge case pairs that pass the High CMC criteria.
The bimodal behavior of the non-match CMC distribution observed in \autoref{fig:cmc_sensitivityScatter} is similar to that of Figure 8 in \citet{chen_convergence_2017} where the High CMC method was applied to cartridge case data from \citet{laPorte_validation_2011}.}

\jz{This illustrates how a prosaic description of an algorithm is generally insufficient to properly implement the method. As is the case with the High CMC method, unintentional ambiguities in written-language descriptions lead to different interpretations of the method. Even precise algorithmic descriptions typically gloss-over the usage of particular programming languages, functions, or parameter settings. Algorithms that theoretically should be consistent across implementations may in-practice yield numerical inconsistencies due to factors that would require the authors' careful consideration to make consistent. For example, how different programming languages handle various datatypes (floating-point, complex numbers, etc.) or how random seeds are used to initialize pseudorandom number generators. Because of this, anything short of providing the code and data used to produce published results renders computational reproduction challenging if not impossible.}

```{r ,echo=FALSE,eval=FALSE, fig.cap='\\label{fig:highCMC_comparison_sensitivity} High CMC count relative frequencies for various interpretations on the High CMC method description given in \\citet{tong_improved_2015} with $T_{\\Delta x} = T_{\\Delta y} = 20$, $T_{\\text{CCF}} = .5$, and $T_{\\theta} = 6$ degrees. In particlar, compares CMC results for different the 4 combinations of \\code{missingTheta\\_decision = "fail"} or \\code{"replace"} and \\code{compareInitialAndHighThetas = TRUE} or \\code{FALSE} (see \\@sec:highMethod for details). Different interpretations of the High CMC method description lead to different results, particularly with respect to the behavior of the non-match CMC count distribution. Additionally, as evidenced by the stillness of the matching CMC count distribution, this illustrates how the High CMC criteria functions essentially as a preliminary classification rule for non-matches vs. matches.',fig.align='left',fig.pos='htbp',out.width='\\textwidth'}
...
\autoref{fig:cmc_sensitivityScatter} shows the results for both decision rules except that the surface matrices were not leveled prior to applying the bandpass Gaussian filter. 
...
<!-- We have observed that the High CMC criteria functions effectively as a preliminary classification rule for matching vs. non-matching cartridge case pairs.  -->
<!-- Any pair that passes the High CMC criteria, matching or non-matching, is assigned a large number of CMCs.  -->
<!-- Thus, the success of the High CMC method hinges on choosing preprocessing parameter and threshold values such that few non-matches pass the High CMC criteria.  -->
...
<!-- While not shown here, we have seen that certain combinations of preprocessing conditions and thresholds yield a bimodal non-match CMC count distribution consisting of pairs that do not pass High CMC criteria (and are assigned few CMCs) and pairs that do (and are assigned many false positive CMCs).  -->
