...
\jz{Each sub-procedure of the CMC method will be described using excerpts from the original papers. A discussion of ambiguities in these descriptions will accompany. Following this, the procedures as implemented in the \code{cmcR} package will be detailed and an exploration of the greater lessons learned while reconciling the methods' ambiguities will be provided.}
...
```{r echo = FALSE,cache = T,fig.align = 'left',fig.cap="\\label{fig:translationVotes} Estimated translation values within 20 pixels of median value for the comparison of (left) Fadul 1-1 and Fadul 1-2 and (right) Fadul 1-1 and Fadul 2-1. 19 out of 29 $(x_i,y_i)$ pairs satisfy the congruency criteria for the matching Fadul 1-1 and Fadul 1-2 pair. In contrast, only 3 out of 35 $(x_i,y_i)$ pairs satisfy the congruency criteria for the non-matching Fadul 1-1 and Fadul 2-1 pair. These examples are illustrative of the more general phenomenon that estimated translation values for non-matching pairs tend to be more diffuse than for matching pairs.",fig.pos = 'h'}
...
# The \code{cmcR} package

## Implementation of preprocessing procedures

\jz{XXX Pretty much this whole preprocessing section will be re-written to reflect the new semi-automatic procedure. Remember to discuss how the inclusion of roll-off tends to inflate the CCF scores due to how "loud" the roll-off is relative to the minute changes in the cartridge case surface. XXX}
...
\jz{XXX Need to tweak cmcPlot function to keep the cartridge case associated with the ordered cells (Fadul 1-1) consistent for each of these 3 plots. XXX}
...
# kmCMC$topVoteCMCs$comparison_2to1 <- bind_rows(kmCMC$topVoteCMCs$comparison_2to1,kmCMC$topVoteCMCs$comparison_2to1)
...
cmcPlots$highCMCs
```
```{r,echo=FALSE,cache = T,fig.cap='\\label{fig:highCMCPlot} CMC results for the comparison between Fadul 1-1 and Fadul 1-2 under the High CMC method using the \\code{cmcPlot} function',fig.align='center',fig.pos='htbp',out.width='1.2\\textwidth',out.extra="trim={0 2.5cm 0 2cm}"}
...
```{r,echo=FALSE,cache = T,fig.cap='\\label{fig:knmCMCPlot} CMC results for the comparison between Fadul 1-1 and Fadul 2-1 under the Top Vote method using the \\code{cmcPlot} function. The High CMC method applied to this pair yielded 0 CMCs (i.e., the pair did not pass the High CMC criterion).', fig.align='center',fig.pos='htbp',out.width='1.2\\textwidth',out.extra="trim={0 2cm 0 1cm}"}
...
\jz{XXX Explain within/between variance ratio. Emphasize need for more specific diagnostic score since so many combinations yield "perfect" classification.}
...
cmcs_varRatios <-  cmcs_downSampled_tighterThreshs %>%
...
cmcs_aucs <- cmcs_downSampled_tighterThreshs %>%
...
\jz{\autoref{fig:highCMC_conservative_sensitivity} illustrates the sensitivity of the High CMC method to various $T_{CCF}$ and $T_{\Delta x}, T_{\Delta y}$ values where $T_{\theta} = 6$ degrees. 
...
```{r,echo=FALSE, fig.cap='\\label{fig:highCMC_conservative_sensitivity} High CMC count relative frequencies for various $T_{\\Delta x}, T_{\\Delta y}$, and $T_{\\text{CCF}}$ values and $T_{\\theta} = 6$ degrees, \\code{missingTheta\\_decision = "fail"}, and \\code{compareInitialAndHighThetas = TRUE}. Thresholds become more stringent as the $T_{\\Delta x}, T_{\\Delta y}$ values decrease and $T_{\\text{CCF}}$ values increase. AUCs based on the minimum CMC count classification threshold and Between/Within group variance ratios are shown for each combination.',fig.align='left',fig.pos='htbp',out.width='\\textwidth'}
...
Also considering \code{compareInitialAndHighThetas = TRUE} vs. \code{FALSE}, which changes whether the $\theta_{\text{ref},1-2}, \theta_{\text{ref},2-1}$ and $\theta_{\text{mode},1-2}, \theta_{\text{mode},2-1}$ values are compared.
...
\jz{XXX Discuss benefits of modularized processing here, including results from not removing tilt.XXX}
...
\jz{XXX Re-iterate points made regarding importance of reproducibility in research. Also discuss challenges in implementing non-open-source methods and lessons learned in implementation (e.g., providing code AND data are necessary, modularization of procedures is best, etc.). XXX}
