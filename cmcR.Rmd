...
\svp{In this paper, we describe the implementation of a general CMC method which encompasses methods described in \citet{song_proposed_2013}, \citet{song_3d_2014}, and \citet{tong_improved_2015}. \jz{XXX We don't implement the method in Chen et al. (2017) and Tong et al. (2018) don't really introduce a new method -- rather, it describes one way of modeling the CMC count distributions.}
\citet{song_proposed_2013} lays out the conceptual framework for the original CMC method, which is referred to as the ``Top Vote" method in \jz{this paper}; \jz{XXX Everyone at NIST just calls the original CMC method "the original method" or something similar -- we decided to call it the "top-vote" to remove the ambiguity of what "original" refers to. XXX}
