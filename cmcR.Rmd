...
\hh{For each cell on the reference scan, a whole set of rotation angles $\theta$, translations $(\delta x, \delta y)$, and cross-correlation values are calculated. These sets need to be aggregated in a single consensus decision. Here, we describe the two methods implemented in `cmcR`: the original decision rule described in  \citet{song_3d_2014} and the High CMC method proposed in \citep{tong_improved_2015}. }
