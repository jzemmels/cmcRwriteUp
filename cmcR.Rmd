...
```{r, echo=FALSE,fig.cap='\\label{fig:cmc_illustration} Illustration of comparing a "cell" in one cartridge case scan to a region in another',fig.align='center',fig.pos='htbp',out.width='.75\\textwidth'}
...
\jz{Based on the definition, the CCF need not be bounded between $-1$ and $1$. However, it is common to normalize the CCF for interpretability and is the convention adopted by the \pkg{cmcR} package. 
...
\svp{The features derived from the correlation cell procedure (CCF$_{max}$, $dx$, $dy$, $\theta$) are used as inputs to the methods for assessment of match strength.}
...
kmComparison$comparison_1to2$ccfResults %>%
  ...
  select(c(`Cell index`,`Pairwise-complete corr.`,`FFT-based corr.`,x,y,theta,`Non-missing percent.`)) %>%
  filter(theta == -24) %>%
  arrange(`Cell index`) %>%
  head() %>%
  knitr::kable(caption = "\\label{tab:cellCCF} Example of output from correlation cell comparison procedure",
               format = "latex",
               align = c("|c","c","c","r","r","c","c|")) %>%
...
\jz{\autoref{fig:translationVotes} shows the distributions of the $(x_i, y_i)$ values which produce the highest CCF for the alignment of each cell/region pair of a known match and known non-match comparison. The squares in \autoref{fig:translationVotes} are centered on the comparisons' respective $(x_{\text{ref}},y_{\text{ref}})$ values and represent the maximum distance an $(x_i,y_i)$ estimated translation value can be from $(x_{\text{ref}},y_{\text{ref}})$ to be classified as "congruent" based on thresholds $T_{x} = T_{y} = 20$ pixels. Of the 43 cell/region pairs considered in the known match comparison, 27 $(x_i,y_i)$ values are within the specified thresholds of $(x_{\text{ref}},y_{\text{ref}})$. In contrast, 13 out of 42 $(x_i,y_i)$ are within the specified thresholds of $(x_{\text{ref}},y_{\text{ref}})$ for the known non-match comparison.}

```{r,echo=FALSE,cache = T}
fadul2.1 <- cmcR::selectBFImpression_sample_x3p("data/fadul2-1.x3p",
                                                gaussFilterWavelength = c(16,500))

knmComparison <- cmcR::cellCCF_bothDirections(x3p1 = fadul1.2$x3p,
                                              x3p2 = fadul2.1$x3p,
                                              cellNumHoriz = 8,
                                              minObservedProp = .1,
                                              regionToCellProp = 9,
...
```{r echo = FALSE,cache = T,fig.align = 'left',fig.cap="\\label{fig:translationVotes} Estimated translation values within 20 pixels of median value for 1 Known Match and 1 Known Non-Match cartridge case pair. The estimated translations tend to be more diffuse for non-matching pairs than matching pairs.",fig.pos = 'h'}
bind_rows(kmComparison$comparison_1to2$ccfResults %>%
            topResultsPerCell() %>%
            ungroup() %>%
            mutate(`Comparison Type` = "Known Match"),
          knmComparison$comparison_2to1$ccfResults %>%
            topResultsPerCell() %>%
            ungroup() %>%
            mutate(`Comparison Type` = "Known Non-Match")) %>%
...
\jz{\autoref{fig:rotationVotes} shows the distribution of the $\theta_i$ values which produce the highest CCF for the alignment of each cell/region pair of the known match and known non-match comparison. A vertical line is drawn at the comparisons' respective $\theta_{\text{ref}}$ value and a threshold of $T_{\theta} = 6$ degrees is used to classify $\theta_i$ values as "congruent." 24 of 43 known match and 13 of 42 known non-match $\theta_i$ values are classified as congruent under these conditions.}

```{r echo = FALSE,cache = T,fig.align = 'left',fig.cap='\\label{fig:rotationVotes} Estimated rotation values within 6 degrees of median value for 1 Known Match and 1 Known Non-Match cartridge case pair. This threshold captures the obvious mode in the Known Match estimated rotations around -24 to -21 degrees. In contrast, no such mode is observed in the Known Non-Match estimated rotations and fewer values are classified as "Congruent" as a result.',fig.pos = 'h'}
bind_rows(kmComparison$comparison_1to2$ccfResults %>%
            topResultsPerCell() %>%
            ungroup() %>%
            mutate(`Comparison Type` = "Known Match"),
  ...
  geom_vline(aes(xintercept = med_theta),
             colour = "#7570b3") +
  # geom_vline(aes(xintercept = theta_threshMinus - .5),
  #            colour = "#7570b3") + 
  # geom_vline(aes(xintercept = theta_threshPlus + .5),
  #            colour = "#7570b3") +
  theme_bw() +
  coord_fixed() +
...
\jz{\autoref{fig:topVoteCMC_sensitivity} illustrates the sensitivity of the Top Vote method to various $T_{CCF}$ and $T_{dx}, T_{dy}$ values where $T_{\theta} = 6$ degrees. The thresholds become more restrictive from top to bottom and from right to left, meaning the least restrictive set of thresholds is represented in the top-right corner. We can see that many of these combinations yield separation between the matching and non-matching CMC count distributions. Additionally, as one might expect, fewer CMCs are assigned to both non-matching and matching pairs as the thresholds become more restrictive.}

```{r echo=FALSE,fig.cap='\\label{fig:topVoteCMC_sensitivity} Top Vote CMC count relative frequencies for various $T_{dx}, T_{dy}$, and $T_{\\text{CCF}}$ values and $T_{\\theta} = 6$ degrees.',fig.align='left',fig.pos='htbp',out.width='1.2\\linewidth'}
load("data/cmcCountData.RData")

cmcs_downSampled %>%
  filter(theta_thresh == 6)  %>%
  filter(round(ccf_thresh,1) %in% c(.3,.4,.5) &
           dx_thresh %in% c(10,20,30)) %>%
  rename(`CCF Threshold` = ccf_thresh,
         `Trans. Threshold` = dx_thresh) %>%
  ggplot(aes(x = topVoteCMCs,y = ..density..)) +
...
```{r,echo=FALSE,message=FALSE,warning=FALSE,cache = T,fig.cap='\\label{fig:kmCMCPerTheta} CMC count per rotation ($\\theta$) values in both directions for a KM cartridge case pair',fig.align='center',fig.pos='h',out.width='.7\\textwidth'}
...
Since the Top Vote and High CMC criterion are applied in both directions for the High CMC method, we determine specifically if $|\theta_{\text{ref},1-2} - \theta_{\text{mode},1-2}| \leq T_\theta$ and $|\theta_{\text{ref},2-1} - \theta_{\text{mode},2-1}| \leq T_\theta$ where the subscripts emphasize the direction considered. 
...
The \code{compareInitialAndHighThetas} argument of the \code{cmcFilter\_improved} function dictates whether this comparison is performed.}

\jz{\autoref{fig:HighCMC_sensitivity} illustrates the sensitivity of the High CMC method to various $T_{CCF}$ and $T_{dx}, T_{dy}$ values where $T_{\theta} = 6$ degrees. 
...
```{r echo=FALSE,fig.cap='\\label{fig:HighCMC_sensitivity} High CMC count relative frequencies for various $T_{dx}, T_{dy}$, and $T_{\\text{CCF}}$ values and $T_{\\theta} = 6$ degrees.',fig.align='left',fig.pos='htbp',out.width='1.2\\linewidth'}

cmcs_downSampled_tighterThreshs %>%
  filter(missingTheta_decision == "fail" & compareInitialAndHighThetas == "TRUE") %>%
  filter(theta_thresh == 6)  %>%
  filter(ccf_thresh %in% c(.3,.4,.5) &
           dx_thresh %in% c(10,20,30)) %>%
  rename(`CCF Threshold` = ccf_thresh,
         `Trans. Threshold` = dx_thresh) %>%
  ggplot(aes(x = highCMCs,y = ..density..)) +
  geom_histogram(aes(fill = type),
                 alpha = .8,
                 colour = "white",
                 binwidth = 1) +
...
This is evidenced in \autoref{fig:HighCMC_sensitivity_relaxed} which shows the CMC count distributions under a less strict set of conditions than those of \autoref{fig:HighCMC_sensitivity}.
In particular, we set \code{missingTheta\_decision = "replace"}, which treats passing the High CMC criterion in one direction as a pass in both, and \code{compareInitialAndHighThetas = FALSE}, which results in no comparison of the $\theta_{\text{ref},1-2}, \theta_{\text{ref},2-1}$ and $\theta_{\text{mode},1-2}, \theta_{\text{mode},2-1}$ values. 
This results in considerably more false positive CMCs assigned to truly non-matching pairs. 
The bimodal behavior of the non-match CMC distribution observed in \autoref{fig:HighCMC_sensitivity_relaxed} is similar to that of Figure 8 in \citet{chen_convergence_2017} where the High CMC method was applied to cartridge case data from \citet{laPorte_validation_2011}.}

```{r echo=FALSE,fig.cap='\\label{fig:HighCMC_sensitivity_relaxed} High CMC count relative frequencies for various $T_{dx}, T_{dy}$, and $T_{\\text{CCF}}$ values and $T_{\\theta} = 6$ degrees.',fig.align='left',fig.pos='htbp',out.width='1.2\\linewidth'}
cmcs_downSampled_tighterThreshs %>%
  filter(missingTheta_decision == "replace" & compareInitialAndHighThetas == "FALSE") %>%
  filter(theta_thresh == 6)  %>%
  filter(ccf_thresh %in% c(.3,.4,.5) &
           dx_thresh %in% c(10,20,30)) %>%
  rename(`CCF Threshold` = ccf_thresh,
...
\jz{XXX Maybe we should (re)move or re-purpose the stuff below? XXX}
...
cmcPlots$initialCMC
```
```{r,echo=FALSE,cache = T,fig.cap='\\label{fig:topVoteCMCPlot} CMC results from a known match comparison under the Top Vote method using the \\code{cmcPlot} function',fig.align='center',fig.pos='htbp',out.width='1.2\\textwidth'}
kmComparison_swapped <- kmComparison
kmComparison_swapped$comparison_1to2 <- kmComparison$comparison_2to1
kmComparison_swapped$comparison_2to1 <- kmComparison$comparison_1to2


kmCMC_swapped <- cmcR::cmcFilter_improved(kmComparison_swapped,
                                          ccf_thresh = .5,
                                          dx_thresh = 20,
                                          dy_thresh = 20,
                                          theta_thresh = 6,
...
cmcPlots$highCMC
```
```{r,echo=FALSE,cache = T,fig.cap='\\label{fig:highCMCPlot} CMC results from a known match comparison under the High CMC method using the \\code{cmcPlot} function',fig.align='center',fig.pos='htbp',out.width='1.2\\textwidth',out.extra="trim={0 2.5cm 0 2cm}"}
...
```{r,echo=FALSE,cache = T,fig.cap='\\label{fig:knmCMCPlot} CMC results from a known non-match comparison under the Top Vote method using the \\code{cmcPlot} function',fig.align='center',fig.pos='htbp',out.width='1.2\\textwidth',out.extra="trim={0 2cm 0 1cm}"}
knmComparison_swapped <- knmComparison
knmComparison_swapped$comparison_1to2 <- knmComparison$comparison_2to1
knmComparison_swapped$comparison_2to1 <- knmComparison$comparison_1to2

knmCMC_swapped <- cmcR::cmcFilter_improved(knmComparison_swapped,
                                           ccf_thresh = .5,
                                           dx_thresh = 20,
                                           dy_thresh = 20,
                                           theta_thresh = 6,
...
cmcR::cmcPlot(x3p1 = fadul2.1$x3p,
              x3p2 = fadul1.1$x3p,
              cellCCF_bothDirections_output = knmComparison_swapped,
              cmcFilter_improved_output = knmCMC_swapped,
              type = "faceted",
...
\jz{XXX A lot of what's below has been moved into the Top Vote and High CMC method sections above. I'm not sure if an official "Results" section is needed based on where we're taking the paper. XXX}
...
```{r echo=FALSE,fig.cap='\\label{fig:topVoteCMC_perfectIdentification} Top Vote CMC count distribution with $T_{\\text{CCF}} = .325$, $T_{dx} = T_{dy} = 25$, and $T_{\\theta} = 6$ degrees.',fig.align='center',fig.pos='htbp',out.width='\\textwidth',out.height=".55\\textwidth"}

barHeights <- cmcs_downSampled_tighterThreshs %>%
  select(topVoteCMCs,type,pairName,ccf_thresh,dx_thresh,theta_thresh) %>%
  distinct() %>%
  filter(theta_thresh ==  6 & ccf_thresh == .325 & dx_thresh == 25) %>%
...
```{r echo=FALSE,fig.cap='\\label{fig:highCMC_perfectIdentification} High CMC count distribution with $T_{\\text{CCF}} = .325$, $T_{dx} = T_{dy} = 25$, and $T_{\\theta} = 6$ degrees',fig.align='center',fig.pos='htbp',out.width='\\textwidth',fig.height=4}
barHeights <- cmcs_downSampled_tighterThreshs %>%
  filter(missingTheta_decision == "fail" & compareInitialAndHighThetas == "TRUE") %>%
  select(highCMCs,type,pairName,ccf_thresh,dx_thresh,theta_thresh) %>%
  distinct() %>%
  filter(theta_thresh ==  6 & ccf_thresh == .325 & dx_thresh == 25) %>%
  mutate(type = factor(type,levels = c("non-match","match"))) %>%
  rename(`CCF Threshold` = ccf_thresh,
         `Trans. Threshold` = dx_thresh) %>%
  group_by(type,highCMCs) %>%
  tally() %>%
  ungroup() %>%
  group_by(type) %>%
  mutate(prop = n/sum(n))
...
cmcs_downSampled_tighterThreshs %>%
  ...
  rename(`CCF Threshold` = ccf_thresh,
         `Trans. Threshold` = dx_thresh) %>%
  ggplot() +
  stat_density(aes(x = highCMCs,y = ..density.., fill = type)) +
  geom_bar(aes(x = highCMCs,fill = type)) +
  facet_wrap(~`Trans. Threshold`,labeller = label_both) +
  geom_text(data = barHeights,
            aes(x = highCMCs,y = n,label = n),
            nudge_y = 7,size = 2.5) +
  scale_fill_manual(values = c("#a60b00","#1b03a3"),
                    aesthetics = c("colour","fill")) +
  guides(fill = guide_legend(title = "Comparison Type",
