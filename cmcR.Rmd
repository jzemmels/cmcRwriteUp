...
\jz{In this section we summarize the ambiguities identified in the original descriptions of the CMC methods. In the following section, we will investigate ways in which such ambiguities can be rectified by proposing a set of reproducibility principles.}

\jz{Critical information is missing from the description of the High CMC method in \citet{tong_improved_2015}.
For one, separate from the description of the decision rule procedure, there is no discussion in \citet{tong_improved_2015} of the procedures used to preprocess the scans.
Because the preprocessing procedures vary in the CMC literature when they are discussed, it is difficult to know how each scan was preprocessed in \citet{tong_improved_2015}.
As we have discussed, this means that the results in \citet{tong_improved_2015} are not reproducible.}

\jz{Additionally, because the High CMC criterion attempts to identify a mode in the CMC-$\theta$ distribution, it is unclear how to proceed if a mode wider than one $\theta$ value is identified (i.e., if two or more consecutive $\theta$ values have CMC counts equal to CMC$_{\max}$).
...
We might behave as if both comparison directions failed the High CMC criterion if one comparison direction fails.
...
Alternatively, we might behave as if both directions pass the High CMC criterion if only one direction passes.
