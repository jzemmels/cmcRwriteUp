...
In this section, we demonstrate the implementation of the decision rules in `cmcR` for both the original method \citep{song_proposed_2013} and the High CMC method  \citep{tong_improved_2015}.
...
Decision rules in `cmcR` are implemented as functions of the form \code{decision\_*}. In particular, the \code{decision\_CMC} function applies both the decision rule of the original method of \citet{song_proposed_2013} and the High CMC method depending on whether  a value for $\tau$ is provided.
...
\autoref{fig:topVoteCMCPlot} shows  congruent matching cells (CMCs) and non-congruent matching cells (non-CMCs) determined under the original method \citep{song_proposed_2013} in blue and red, respectively.
...
The bottom row shows the 15 CMCs and 15 non-CMCs when the roles are reversed.
...
Only one cell is classified as a congruent matching cell under the original method of \citet{song_proposed_2013}  when Fadul 1-1 was treated as the reference scan. No cells were classified as CMCs in the other direction.
...
We have applied our implementation of these two methods to the 780 cartridge case pairs available in the \citet{fadul_empirical_nodate} data set under a wide variety of processing conditions to determine which conditions yield \hh{the best results and match the most with the published results}.
