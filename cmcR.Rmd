...
\svp{In the remainder of this paper, we describe} a general, \svp{reproducible, and open-source} CMC pipeline which encompasses \jz{those} discussed in \citet{song_proposed_2013}, \citet{song_3d_2014}, and \citet{tong_improved_2015}.
...
We have demonstrated that the three common reasons for unreproducible results: ambiguity in how procedures are implemented, missing or incomplete data, and missing or incomplete code, are all applicable to the Congruent Matching Cells pipeline.
...
However, we have observed that the conceptual description provides only enough detail to qualitatively reproduce the original results.
To truly compare two different algorithms, including two different implementations of the same algorithm, at a minimum comparable results must be provided in supplementary material.
However, even comparable results do not ensure full computational reproducibility.}

\jz{\textbf{Describe factors that cause unreproducibile results if only conceptual descriptions or pseudocode are provided.}}

\jz{When translating an algorithm from a description, reproducibility of results is challenging as an implementation may depend on the creator's programming habits or the programming language used.
...
\jz{Open-source algorithms and data, accompanied with documentation detailing language/package versions, random seeds, metadata, etc., most clearly ensures fully reproducible results.}
```
...
```{=tex}
\jz{The Daubert Standard requires that a scientific method be widely accepted by a relevant scientific community before it can be used in an expert witness's testimony \citep{daubert}.
