...
\jz{Cartridge case scans are commonly stored in the ISO standard x3p file format \citep{ISO25178-72}. An x3p consists of a single surface matrix representing the height value of the surface at a particular location and metadata concerning the parameters under which the scan was taken (size, resolution, author, etc.). The \pkg{x3ptools} package can be used to work with these x3p files in R \citep{x3ptools}.
}
