...
cmcs_downSampled %>%
  ...
  filter(round(ccf_thresh,1) %in% c(.4,.5) &
...
cmcs_downSampled_tighterThreshs %>%
  ...
  filter(theta_thresh == 6)  %>%
  filter((ccf_thresh == .5 | (ccf_thresh > .39 & ccf_thresh < .41)) &
           dx_thresh %in% c(10,20,30)) %>%
  rename(`CCF Threshold` = ccf_thresh,
         `Trans. Threshold` = dx_thresh) %>%
...
```{r echo=FALSE,fig.cap='\\label{fig:topVoteCMC_perfectIdentification} Top Vote CMC count distribution with $T_{\\text{CCF}} = .6$, $T_{dx} = T_{dy} = 20$, and $T_{\\theta} = 6$ degrees.',fig.align='center',fig.pos='htbp',out.width='\\textwidth',out.height=".55\\textwidth"}
...
```{r echo=FALSE,fig.cap='\\label{fig:highCMC_perfectIdentification} High CMC count distribution with $T_{\\text{CCF}} = .6$, $T_{dx} = T_{dy} = 20$, and $T_{\\theta} = 6$ degrees',fig.align='center',fig.pos='htbp',out.width='\\textwidth',fig.height=4}
barHeights <- cmcs_downSampled_tighterThreshs %>%
  filter(missingTheta_decision == "fail" & compareInitialAndHighThetas == "TRUE") %>%
