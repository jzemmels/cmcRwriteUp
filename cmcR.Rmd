...
nrbtd_url <- "https://tsapps.nist.gov/NRBTD/Studies/CartridgeMeasurement/"
download.file(file.path(nrbtd_url, fadul1.1_id), destfile = "data/fadul1-1.x3p", mode = "wb")
download.file(file.path(nrbtd_url, fadul1.1_id), destfile = "data/fadul1-2.x3p", mode = "wb")
```
...
During a forensic examination, \hh{two pieces of ballistic evidence} are  placed under a \dfn{comparison microscope}. Comparison microscopes allow for a \hh{side-by-side} comparison of two objects within the same viewfinder, as seen in
Figures \ref{fig:ccPair_separated} and \ref{fig:ccPair_combined}, which show two pairs of breech faces. In \autoref{fig:ccPair_separated} the pair is unaligned, in \autoref{fig:ccPair_combined} the cartridge cases are aligned. 
