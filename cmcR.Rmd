...
\begin{figure}\label{fig:overview-flow}
\centering
...
As discussed in section REF, neglecting to remove such trends leads to very different results based on our implementation. 
For example, two non-match cartridge cases with similar trends might be incorrectly deemed "congruent." }
...
seft in cartridge cases [\citet{hough}, \citet{gerules_survey_2013}]^[\svp{Some firearms leave a rectangular firing pin impression; this method could be modified to account for those models, but we have not yet expanded our automated processing to these types of firearms.}].  -->
...
\svp{xxx This probably should be a pipeline, not a single function call. But that's just my opinion. Also, this still has ransac parameters, which we might want to get rid of as we're no longer explaining them XXX} \jz{XXX Yes, we've switched this function out for a different one. I think this was just left here from the old version -- will delete.XXX}
...
The High CMC method was proposed by \citet{tong_improved_2015} to address situations in which the \jz{original method of \citet{song_proposed_2013}} is excessively strict. 
...
\autoref{fig:knmCMCPerTheta}, on the other hand, shows the CMC counts for a known non-match pair; in this comparison, no such CMC count mode is achieved.

\citet{tong_improved_2015} introduce an additional criterion to identify a mode in the CMC count per $\theta$ distribution. 
Namely, they \svp{define} a "high" CMC threshold defined to be CMC$_{\text{high}} =$ CMC$_{\max} - \tau$ for some constant $\tau$ (they choose $\tau = 1$) where CMC$_{\max}$ is the maximum CMC count attained across all rotation values considered. 
