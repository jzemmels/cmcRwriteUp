...
\svp{Reproducibility is an indispensable component of scientific validity\citep{goodman_what_2016}. 
...
Our contribution to the CMC literature is the open-source implementation, which removes any descriptive ambiguity in the human-friendly descriptions in the original papers; we provide more detailed, hardware-friendly code. 
While we have largely succeeded in reproducing the published results of the CMC algorithm, some uncertainty remains that our pipeline is similar enough to reproduce any results on an external data set \citep{vanderplasComparisonThreeSimilarity2020}. 
It is worth considering the obstacles we encountered, because they are common when attempting to implement a solution to paper published without code, pseudocode, or data, as documented in \ref{reprod-defn}.}

\jz{\textbf{Paragraph 1: Refer back to hierarchy of algorithmic transparency. Assert that we currently have a conceptual description when we really need comparable results (at a minimum) or, optimally, fully reproducible results.}}
...
\jz{\textbf{Paragraph 2: Discuss how the manner in which the pipeline is currently presented doesn't lead to community-wide acceptance. This isn't just important from a scientific standpoint, there is a legal imperative (Daubert) to achieving community-wide acceptance.}}
...
\jz{\textbf{Paragraph 3: Describe how our implementation can be used as a foundation for further improvements upon the CMC method and how the package overall can be used as a template for future open-source implementations.}}
...
<!-- The results shared in this manuscript indicate that the implementation of the CMC pipeline in the \CRANpkg{cmcR} package is qualitatively similar to those published in \citet{song_3d_2014}, \citet{tong_improved_2015}, \citet{chen_convergence_2017}, and \citet{song_estimating_2018}. -->

```{=tex}
\jz{Our implementation serves as an example of the barriers which occur when translating textual descriptions of algorithms into detailed source code.
This process offers some insight into the necessary components which must be available for full reproducibility of results.
...
The failure to ensure full reproducibility not only imposes a general barrier to scientific progress; in this application, it also highlights the crucial need of open-source, transparent methodology in high-stakes fields such as forensic analysis.
