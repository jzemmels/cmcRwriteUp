...
\jz{Only cells with a minimum proportion of non-missing pixels are }\svp{assessed}. \jz{The minimum threshold differs across CMC papers \citep{chen_convergence_2017,song_estimating_2018}}\svp{, as shown in \autoref{fig:cmc-schematic}, and is referenced but not specified in several other papers \citep{tong_fired_2014,song_3d_2014,chu_validation_2013}.} \svp{XXX what proportion do we use? If it's variable, we should say that we have implemented this as a parameter setting for compatibility with the other algorithms}
...
\jz{\autoref{fig:translationVotes} shows the distributions of the $(x_i, y_i)$ values which produce the highest CCF for the alignment of each cell/region pair of a known match and known non-match comparison. 
...
Of the 43 cell/region pairs considered in the known match comparison, 27 $(x_i,y_i)$ values are within the specified thresholds of $(x_{\text{ref}},y_{\text{ref}})$. 
In contrast, 13 out of 42 $(x_i,y_i)$ are within the specified thresholds of $(x_{\text{ref}},y_{\text{ref}})$ for the known non-match comparison.}
...
bind_rows(kmComparison$comparison_1to2$ccfResults %>%
  ...
  theme_bw() +
  coord_fixed() +
  scale_x_continuous(expression(paste("Estimated rotation angle ", theta)), breaks = seq(-30,30,by = 3)) +
  scale_y_continuous("# Cell Pairs") + 
...
```{r echo=FALSE,fig.cap='\\label{fig:topVoteCMC_sensitivity} Top Vote CMC count relative frequencies for various $T_{dx}, T_{dy}$, and $T_{\\text{CCF}}$ values and $T_{\\theta} = 6$ degrees.',fig.align='left',fig.pos='htbp',out.width='\\textwidth'}
