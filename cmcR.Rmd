...
The \citet{nas_2019} defines \dfn{reproducibility} as  "obtaining consistent computational results using the same input data, computational steps, methods, code, and conditions of analysis."
...
In many situations, this level of reproducibility is not provided \svp{-- not just in forensics but in many other applied disciplines.} 
\svp{In forensics in particular, it is easier to list the exceptions: algorithms which have been proposed that are reproducible in subdiciplines like} DNA \citep{Tvedebrink2020,osiris}, glass \citep{Curran2000-hp,Park2019}, handwriting \citep{crawford_handwriting_2020}, shoe prints \citep{park_algorithm_2020}, and ballistic evidence \citep{hare_automatic_2016,tai_fully_2018}).

We find it useful to \svp{instead} consider a more inclusive hierarchy of reproducibility.
Algorithms at higher tiers of the algorithm are more easily reproducible in the sense that fewer resources are required to \svp{(re)-}implement the algorithm.}
...
In contrast, implementing fully reproducible algorithms requires only as much time as it takes to emulate the original \svp{development} environment.
...
The Congruent Matching Cells (CMC) pipeline is a collection of algorithms developed by a group of researchers at the National Institute of Standards and Technology (NIST) to process and compare cartridge case evidence \citep{song_proposed_2013}. Since then, the pipeline and its extensions \citep{tong_improved_2015,chen_convergence_2017,song_estimating_2018} have shown promise in being able to differentiate between matching and non-matching cartridge cases.
...
\svp{In our hierarchy of reproducibility, this puts the CMC algorithm somewhere between the conceptual description and reproducible data stage: the steps are described but no code is available, and the raw data are available but manual preprocessing steps make this raw data insufficient to replicate the pipeline even with newly written code.}
...
<!-- The numerous publications since the initial CMC method proposal in \citet{song_proposed_2013} reflect that its creators and collaborators are committed to improving the method. -->
<!-- Despite these improvements, we recognize that the method is not, and perhaps never will be, a "completed" algorithm. -->
<!-- This is an ethic that the forensics community at-large should strive to emulate.  -->
<!-- Especially in such a high-stakes application, it is imperative that we continually seek incremental improvements upon previously-developed work. -->
<!-- Therefore, open-sourcing code should be considered essential in forensics as it both enables and encourages others to validate, experiment with, and improve upon previous work. -->
<!-- This accelerates the speed of development which leads to more effective algorithms and more accurate evidentiary conclusions. -->
<!-- There is already precedence for open-sourcing NIST-developed code as evidenced by the hundreds of public Github repositories accessible at <https://github.com/usnistgov>. -->
...
Additionally, we argue that our implementation can be used as a template for future implementations of forensic pattern-matching algorithms to not only ensure transparency and auditability, but also to \svp{facilitate incremental improvements in forensic algorithms}.
...
<!-- Despite using the same validation data from \citet{fadul_empirical_2011} across papers, current authors do not provide justification for changing these parameters.  -->
<!-- At the same time, different parameter choices have been shown to yield very different results; even when applying the same decision rule. -->
<!-- This sensitivity of various pipelines to different conditions has not previously been discussed in detail. -->
...
<!-- \jz{XXX Discuss from the ground-up what needs to be done to pre-process the scans. -->
<!-- Include quote about "trimming" the scans and how we preferred to implement this automatically for the sake of repeatability. -->
<!-- Then, summarize this automatic implementation by trimming down the implementation section.} -->

<!-- \jz{"Trimming" seems to encompass: preProcesscrop, preProcessremoveTrend} -->

<!-- \jz{Removal of outliers: use a lowpass filter?} -->

<!-- \jz{preProcessgaussFilter.} -->
...
\hline
\end{tabular}
\caption{Description of pre-processing procedures from \citet{song_3d_2014} vs. considerations that need to be made when implementing these procedures. Each of these considerations requires the implementer to decide between potentially many possible choices.}
\label{tab:preProcessTable}
\end{table}
```


<!-- \jz{XXX Start CMC-specific criticism} -->
...
\svp{As discussed in the introduction, it is difficult to replicate manual steps as part of a reproducible pipeline; the best solution is for the authors to provide intermediate data after the manual steps have been completed.}

<!-- No further information is given in the paper describing what criteria were used for this process; nor are the trimmed scans provided as intermediate data. -->
...
<!-- Further, there are various parameters requiring specification to implement an outlier imputation procedure or Gaussian regression filter and it is not clear how these parameters were chosen \citep{brinkman_bodschwinna_2003}. -->
...
<!-- \jz{XXX End CMC-specific criticism} -->
...
A direct calculation of the CCF for breech face scans based on the definition above is \svp{prohibitively} slow.
...
<!-- \jz{XXX Start CMC-specific criticism here} -->
...
<!-- \jz{XXX This whole section could be viewed as criticism. Should this be outright removed?} -->
...
\svp{While many of these choices are unlikely to affect the results dramatically,} we believe that any amount of variability that exists solely because of uncertainty in how the method was intended to be implemented is both unnecessary and dangerous in \svp{this application}.
...
\svp{During the creation of the cmcR package}, the process of re-implementing the comparison and decision steps of the pipeline was fairly straightforward.
...
<!-- Ambiguities in the pipelines range from minor implicit parameter choices (e.g., the convergence criteria for the robust Gaussian regression filter \citep{brinkman_bodschwinna_2003}) to procedures that fundamentally change how similarity features are extracted and compared (e.g., how the cross-correlation is calculated). -->
<!-- As discussed in the [introduction](#intro), this is an issue that pervades any computational research for which only verbal descriptions of algorithms are provided. -->
<!-- The only solution to such ambiguity is to enumerate, implement, and pare-down the possible choices that could have been made to arrive to published results. -->
<!-- Unsurprisingly, this process takes a considerable amount of time and resources that would be better spent furthering the state of the field. -->
...
\svp{Through the development of the cmcR package, we have demonstrated} the ambiguities in the [CMC pipeline descriptions](cmcMethod) and lack of guidance in the choice of parameter settings propagate through the algorithm and produce highly variable results.
...
\svp{Next,} we focus on a discussion of the sensitivity of some of the pipeline's parameters.
...
<!-- \jz{XXX End CMC-related criticism} -->
...
<!-- \jz{XXX Start CMC-related criticism} -->

As shown in \autoref{fig:cmc_sensitivityScatter}, de-trending breech-scans in the preprocessing stage emerges as a \svp{critical} step to achieve good algorithmic results.
...
<!-- \jz{XXX I think shifting the focus from "the CMC method is [xyz]" to "forensic algorithms in-general tend to be [xyz], and the CMC method is no exception" would soften the tone slightly.} -->
...
Consequently, there are multiple choices that \svp{are compatible with the description} of an algorithm \svp{in a publication}.
...
<!-- The CMC algorithm highlights the importance of open software and full reproducibility: the algorithm is not useful in the context of the wider problem it is hoping to solve unless there is a software implementation and documented results on data provided for general consumption. -->
...
<!-- The most troublesome portion of the process was in replicating the manual preprocessing of the scans before the algorithmic steps were applied: this step in particular could have been easily avoided had the authors shared the manually processed data along with their paper.  -->
...
\svp{In addition, because we have structured the \CRANpkg{cmcR} implementation as a modular pipeline, it is easier to improve upon the CMC method and document the effects of specific changes to the algorithm compared to previous versions.}
...
<!-- \jz{Nonetheless,} it is possible that our pipeline may not sufficiently reproduce the original authors' results on an external data set, which is an important assessment for any computational algorithm \jz{and should be explored further in the future} \citep{vanderplasComparisonThreeSimilarity2020}. -->

<!-- While we have largely succeeded in qualitatively reproducing the published results of the CMC algorithm, some uncertainty remains.  -->
<!-- It is possible that our pipeline may not sufficiently reproduce the original authors' results on an external data set, which is an important assessment for any computational algorithm \citep{vanderplasComparisonThreeSimilarity2020}. -->
<!-- It is worth considering the obstacles we encountered during the implementation, because they are common when attempting to recreate the computational steps in a paper published without code, pseudocode, or data, as described in \autoref{reprod-defn}. -->
