...
## ----preprocessing-schematic,echo=FALSE,fig.cap="Overview of the set of preprocessing steps used in the CMC algorithms. Where a procedure step is not discussed or explicitly not applied in the paper, the path traverses empty space.",fig.align="center",fig.fullwidth=TRUE----
knitr::include_graphics("images/preprocessing_flow.png")


## ----preProcessTable_latex,echo=FALSE,eval=knitr::is_latex_output()-----------
...
## ----preProcessTable_html,echo=FALSE,eval=knitr::is_html_output()-------------
#> data.frame("Description from Tong et al. (2014)" =
#>              c('"Trim off the inside firing pin surface and other areas outside the breech face mark, so that only breech face impression data remain for correlation."',
#>                '"Identify and remove dropouts or outliers."',
#>                '"Apply a band-pass Gaussian regression filter with 40 $\\mu$m short cutoff length and 400 $\\mu$m long cutoff length to remove low frequency components, including surface curvature, form error, waviness and high frequency components which mainly arise from the instrument noise."'),
...
## ----processingPipeline, echo = F,warning = F,message = F,cache = T,fig.cap='Illustration of the  preprocessing pipeline implemented in \\CRANpkg{cmcR}.  At each stage, the variability in height across the scan decreases as extraneous sources of noise are removed.',fig.align='center',fig.pos='htbp',fig.fullwidth = TRUE----
...
## ----processedScans,echo=FALSE,cache = T,fig.cap='Fadul 1-1 and Fadul 1-2 after preprocessing. Similar striated markings are now easier to visually identify on both surfaces. It is now clearer that one of the scans needs to be rotated to align better with the other.',fig.align='center',fig.pos='htbp',fig.fullwidth=TRUE, message = F, warning = F----
...
## ----cellCCF-latex, echo=FALSE,warning=F,message=F,eval=knitr::is_latex_output(),cache = F----
...
## ----cellCCF-html,echo=FALSE,warning=F,message=F,eval=knitr::is_html_output(),cache = F----
#> kmComparisonFeatures %>%
#>   mutate(`Cell index` = cellIndex,
#>          `Pairwise-complete corr.` = round(pairwiseCompCor,3),
#>          `FFT-based corr.` = round(fft_ccf,3)) %>%
...
## ----thresholdTable_latex,echo=FALSE,eval=knitr::is_latex_output()------------
data.frame("Paper" = 
...
## ----thresholdTable_html,echo=FALSE,eval=knitr::is_html_output()--------------
#> data.frame("Paper" =
#>              c("Song et al. (2014)","Tong et al. (2014)","Tong et al. (2015)",
#>                "Chen et al. (2017)","Song et al. (2018)"),
#>            "Translation $T_x, T_y$ (in pixels)" = c(20,30,15,20,20),
...
## ----decisionRuleSensitivity-comparison, echo=FALSE,fig.cap='CMC count relative frequencies under the original decision rule and the High CMC decision rule for $T_{\\Delta x} = 20 = T_{\\Delta y}$ pixels, $T_{\\text{CCF}} = 0.5$, and $T_{\\theta} = 6$ degrees. AUC $= 1.00$ corresponds to perfect separation of the match and non-match CMC count distributions. We can see that, for this set of processing parameters, the High CMC decision rule yields higher CMC counts for known matches that the original decision rule while known non-matches have the same distribution under both methods.', fig.align='center',fig.pos='htbp',fig.fullwidth=TRUE----
