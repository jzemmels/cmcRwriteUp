...
A summary of these steps is shown in Figure \@ref(fig:preprocessing-schematic).
...
For example, Table `r knitr::asis_output(ifelse(knitr::is_html_output(), '\\@ref(tab:preProcessTable-html)', '\\@ref(tab:preProcessTable)'))` compares the preprocessing procedures as described in @song_3d_2014 to considerations that need to be made when implementing the procedures.
...
```{r preProcessTable,echo=FALSE,eval=knitr::is_latex_output()}
...
```{r preProcessTable-html,echo=FALSE,eval=knitr::is_html_output()}
data.frame("Description from Song et al. (2014)" = 
             c('"Trim off the inside firing pin surface and other areas outside the breech face mark, so that only breech face impression data remain for correlation."',
               '"Identify and remove dropouts or outliers."',
               '"Apply a band-pass Gaussian regression filter with 40 $\\mu$m short cutoff length and 400 $\\mu$m long cutoff length to remove low frequency components, including surface curvature, form error, waviness and high frequency components which mainly arise from the instrument noise."'),
  ...
  knitr::kable(col.names = c("Description from Song et al. (2014)",
                             "Implementation Considerations"),
               caption = "Description of preprocessing procedures from @song_3d_2014 vs. considerations that need to be made when implementing these procedures. Each of these considerations requires the implementer to decide between potentially many choices.",
...
```{r thresholdTable-latex,echo=FALSE,eval=knitr::is_latex_output()}
data.frame("Paper" = 
...
```{r thresholdTable-html,echo=FALSE,eval=knitr::is_html_output()}
data.frame("Paper" = 
             c("Song et al. (2014)","Tong et al. (2014)","Tong et al. (2015)",
               "Chen et al. (2017)","Song et al. (2018)"),
           "Translation $T_x, T_y$ (in pixels)" = c(20,30,15,20,20),
...
Figure \@ref(fig:decisionRuleSensitivity-comparison) shows results for the original decision rule and the High CMC decision rule for a parameter setting of $T_x = 20 = T_y$ pixels, $T_{\text{CCF}} = 0.5$, and $T_{\theta} = 6$.
...
Figure \@ref(fig:cmc-sensitivityScatter) shows the value of the variance ratio under various settings.
...
Interestingly, as seen in Table `r knitr::asis_output(ifelse(knitr::is_html_output(), '\\@ref(tab:thresholdTable-html)', '\\@ref(tab:thresholdTable-latex)'))`, only the parameters for the High CMC decision rule discussed in @song_estimating_2018 fall into these ranges.
...
As shown in Figure \@ref(fig:cmc-sensitivityScatter), de-trending breech-scans in the preprocessing stage emerges as a critical step to achieve good algorithmic results.
...
Figure \@ref(fig:cmc-varRatioComparison) shows variance ratio values based on results presented in various CMC papers using data from @fadul_empirical_2011 and @weller_2012.
...
However, as discussed previously, it is difficult to justify differences in these values because the various papers different preprocessing procedures and parameters (e.g., see Table `r knitr::asis_output(ifelse(knitr::is_html_output(), '\\@ref(tab:thresholdTable-html)', '\\@ref(tab:thresholdTable-latex)'))`).
The last two rows show the largest variance ratio values for the original and High CMC decision rules based on the grid search summarized in Figure \@ref(fig:cmc-sensitivityScatter).
