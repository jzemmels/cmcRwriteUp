...
**We respectfully disagree with the reviewer's claim that the CMC method sensitivity is "well known." Previous studies do not provide a sensitivity analysis to demonstrate how various processing decisions affect the final CMC scores. We can only surmise as much by comparing results across CMC papers where various parameters inexplicably change even though the same NIST authors apply the same methods (e.g., the original or High CMC decision rules) to the same data set (the Fadul scans). One of our goals in this paper was to provide the first demonstration of the sensitivity of the final CMC scores to (a limited grid of) processing decisions and argue that such a sensitivity discussion is necessary in any forensic pattern-matching algorithm proposal.**
