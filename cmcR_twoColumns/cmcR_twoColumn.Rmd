...
In \citet{song_3d_2014}, the process is described with characteristic brevity:
...
Using the thetas, translation vectors, and CCFs produced by the correlation cell procedure, the CMC literature describes two various methods for aggregating this information into a consensus decision about the similarity of the two cartridge cases.
...
We briefly describe the decision rule of the original method of \citet{song_proposed_2013} here (the High CMC method of \citet{tong_improved_2015} is not discussed due to space - 
...
The \texttt{decision\_CMC} applies the decision rule of the original method of \citet{song_proposed_2013}.
...
We have applied our implementation of these two methods to the 780 cartridge case pairs in the Fadul data set under a variety of processing conditions.
...
Conditional on this, choosing threshold $T_x, T_y \geq 15$, $T_\theta = 6$ and $T_{\text{CCF}} \in [.4,.5]$ approximately yields the most promising variance ratios.
...
These are similar thresholds to those chosen in other CMC papers (see \autoref{fig:cmc_sensitivityScatter} in the Appendix for variance ratios for both $\theta = 3, 6$ degrees under both decision rules available in the \textbf{cmcR} package.)
