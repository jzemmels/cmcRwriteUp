...
{\textcolor{orange}{In the following sections we break each of these stages further into a set of modular steps}}; allowing for testing of new variations against the old in a coherent, unified framework.
% {\textcolor{orange}{One advantage of modularizing}} these algorithms is that we can implement an algorithm as a set of {\textcolor{orange}{sequential}} procedures.
% {\textcolor{orange}{This allows us to}} then test new variations against the old {\textcolor{orange}{implementation}} in a coherent, unified framework.

% The primary difference between the original and high CMC methods
% {\textcolor{orange}{lies in how}} the decision rules are utilized to
...
% The surface matrices are approximately \(1200 \times 1200\) pixels in size {\textcolor{orange}{corresponding to an area of about $3.8 \times 3.8$ mm$^2$}}.
...
{\textcolor{purple}{We demonstrate usage of the \texttt{preProcess\_*} functions on the Fadul 1-1 scan. 
...
% Typically, in the CMC literature,}} every other row and column in the surface matrix {\textcolor{blue}{is retained}}, resulting in a matrix that is approximately 25\% of the original scan dimension. 
...
% Step (1) in \autoref{fig:processingPipeline} shows an {\textcolor{purple}{unprocessed breech face scan.}}
...
% height values. In
% {\textcolor{purple}{steps (1) and (2) of \autoref{fig:processingPipeline}}}
% {\textcolor{blue}{it is clear that there is a}}
% {\textcolor{purple}{southwest-to-northeast trend in height values. 
...
% {\textcolor{purple}{Neglecting to remove such trends leads to very different results based on our implementation. 
% For example, two different-source cartridge cases with similar trends might be incorrectly deemed "congruent." }}
% {\textcolor{blue}{{\textcolor{purple}{For authors that use something other than a Gaussian regression filter, i}}t is unclear how such trends in height values are dealt with; certainly, a high-pass Gaussian filter will remove some structure, but it is not explicitly mentioned whether any leveling is performed manually as well.}}
...
% {\textcolor{blue}{In the final preprocessing step,}} a band-pass filter
...
% For example, low frequency (high wavelength) global structure may exist in a cartridge case scan due to manufacturing specifications {\textcolor{blue}{(In forensics, this type of structure is referred to as a class characteristic -- it is shared by many objects of the same make and model)}}.
% {\textcolor{purple}{Such similarities are often used by forensic practitioners to initially pare down the possible pool of matching cartridge cases \citep{firearm_id_thompson}.}}
...
% A band-pass Gaussian filter combines the effects of low and high-pass filters{\textcolor{blue}{; we have chosen to implement it by applying a low pass filter and a high pass filter consecutively.}}
...
% {\textcolor{blue}{in part because the CMC literature contains limited details on outlier removal. We instead rely on the low-pass portion of the Gaussian filter to reduce the effects of any high-frequency noise.}}

% {\textcolor{purple}{\autoref{fig:processedScans} displays the processed Fadul 1-1 and Fadul 1-2 scans}}; {\textcolor{blue}{the second matrix is processed using the same function arguments}}.
...
% If a large {\textcolor{blue}{proportion}} of these correlation cells are deemed highly similar to regions in the other breech face scan, then there is evidence that the two cartridges cases match. 
The number of highly similar cells, the \textit{CMC count}, can be used as a more {\textcolor{blue}{robust}} similarity metric, {\textcolor{blue}{compared to a score derived from the entire cartridge case}}. The cell size (and thus, the corresponding number of cells) is optimized experimentally:
...
{\textcolor{blue}{optimal translation}} needed to align \(B\) with
\(A\).

{\textcolor{blue}{In \citet{song_3d_2014}, the process is described with characteristic brevity:}}
...
% {\textcolor{blue}{there are no details provided in the}} CMC literature
% {\textcolor{blue}{indicating which interpolation algorithm was used}}.
% {\textcolor{blue}{In \textbf{cmcR}, s}}urfaces matrices are rotated using a
% "nearest-neighbor'' interpolation scheme \citep{imager}.

% {\textcolor{blue}{For clarity and reproducibility, we will attempt to provide more precise mathematical and computational descriptions of the algorithm, starting with a definition of the cross-correlation function and later discuss more efficient computational implementations}}.
% For two real-valued, \(M \times N\) matrices \(A\) and \(B\), the
% cross-correlation function, denoted \((A \star B)\) can be defined as \[
% (A \star B)[m,n] = \sum_{i} \sum_{j} A[i,j] B[(i + m), (j + n)].
% \] By this definition, the resulting CCF is a matrix whose \([m,n]\)th
...
% {\textcolor{blue}{optimal translation}} needed to align \(B\) with
% \(A\).
% {\textcolor{purple}{The CCF as defined need not be bounded between $-1$ and $1$. However, it is common to normalize the CCF for interpretability, and}}
% {\textcolor{blue}{this}}
% {\textcolor{purple}{is the convention adopted by the \textbf{cmcR} package. 
...
{\textcolor{blue}{Published implementations of the CMC algorithm do not describe precisely how the CCF is calculated. 
...
% {\textcolor{blue}{This implementation leverages the Cross-Correlation Theorem, which states that for matrices $A$ and $B$,}}
% \[
...
% {\textcolor{blue}{While it is unclear how the CCF is implemented in the CMC papers (it is not even defined mathematically), the \textbf{cmcR} package adopts the following conventions:}}

% \begin{itemize}
% \item
%   {\textcolor{blue}{Missing values are replaced with the overall mean value when the FFT-based CCF is computed {\textcolor{purple}{(via the \texttt{comparison\_replaceMissing} function)}}.}}
% \item
%   {\textcolor{blue}{The optimal translation is determined using the FFT-based CCF {\textcolor{purple}{(via the \texttt{comparison\_fft\_ccf} function)}}. Replacing the missing values with the overall mean leads to a deflated CCF value, but produces an accurate estimate of the optimal translation.}}
% \item
%   {\textcolor{blue}{Using the optimal translation determined from the FFT-based CCF, we compute the pairwise complete CCF directly, avoiding any distortion of the CCF computation based on compensation for missing values {\textcolor{purple}{(via the \texttt{comparison\_cor} function)}}.}}
% \end{itemize}

% {\textcolor{purple}{The code in \autoref{fig:comparisonCode} demonstrates how the \texttt{comparison\_allTogether} function can be used to perform the entire cell-based comparison procedure in one call. 
...
{\textcolor{purple}{When extracting similarity features, only cells with a proportion of missing pixels below a user-defined threshold are }}{\textcolor{blue}{assessed}} (85\% in \autoref{fig:comparisonCode}).
...
% {\textcolor{purple}{The \texttt{comparison\_calcPropMissing} function computes the proportion of a matrix that is missing (\texttt{NA}-valued).}}
...
{\textcolor{blue}{The \texttt{comparison\_allTogether} wrapper function performs the following steps:}}
...
% {\textcolor{blue}{The features derived from the correlation cell procedure (CCF$_{max}$, $x$, $y$, $\theta$) are used as inputs to the methods for assessment of match strength.}}
...
{\textcolor{blue}{Using the thetas, translation vectors, and CCFs produced by the correlation cell procedure, the CMC literature describes two slightly different methods for aggregating this information into a consensus decision about the similarity of the two cartridge cases.}} As stated in \citet{song_3d_2014},
...
% A truly matching pair of cartridge cases {\textcolor{blue}{would have similar translation and rotation values for each cell, estimated by maximizing the cell-wise CCF; a nonmatching pair of cartridge cases would have a variety of different translation and rotation values showing no particular pattern.}}
...
% {\textcolor{purple}{Where the various CMC decision rules principally differ is in how they identify a "consensus" among the estimated $x,y, \theta$ values.}}

We briefly describe the two {\textcolor{purple}{decision rules}} implemented in \textbf{cmcR}: the original method of \citet{song_proposed_2013} and the High CMC method \citet{tong_improved_2015}.\footnote{For a more thorough explanation of the procedures, refer to the \href{https://csafe-isu.github.io/cmcR/articles/decisionRuleDescription.html}{CMC Decision Rule Description} vignette in the \textbf{cmcR} package.}
...
% {\textcolor{purple}{This section briefly describes the decision rule of the original method of \citet{song_proposed_2013}. For a thorough explanation of the procedure, refer to the}}
% \href{https://csafe-isu.github.io/cmcR/articles/decisionRuleDescription.html}{CMC
% Decision Rule Description}
% {\textcolor{purple}{vignette in the \textbf{cmcR} package.}}

{\textcolor{purple}{An actual implementation of the original method of \citet{song_proposed_2013} is described in \citet{song_3d_2014}. The decision rule}}
\citet{song_3d_2014} describe using {\textcolor{purple}{is based on}}
...
% {\textcolor{blue}{Then, the distances between the consensus registration values and the cell comparison values are assessed to determine whether they are within a specified distance of the consensus.}}
% {\textcolor{blue}{This consensus assessment introduces threshold parameters $T_{x}, T_{y}, T_\theta, T_{\text{CCF}}$.}}

% {\textcolor{blue}{Let $x_i, y_i, \theta_i$ denote the translation and rotation parameters which produce the highest CCF for the alignment of cell/region pair $i$. A c}}ell/region
...
% {\textcolor{purple}{The High CMC method of \citet{tong_improved_2015} lifts the single-choice restriction by allowing cells to cast a vote for the translation phase at every $\theta$ value for which it has a sufficiently large associated CCF$_{\max}$ value. 
...
% {\textcolor{purple}{A single cell/region pair may be considered congruent for multiple $\theta$ values and thus included among multiple CMC$_{\theta}$ counts.
...
% {\textcolor{purple}{The empirical assumption underlying the High CMC method is that the number of cells classified as congruent should be larger near the true $\theta$ value (the "initial phase angle $\Theta_0$", as they call it) than for other $\theta$ values if the cartridge case pair is indeed a match. A "prominent peak" or mode should be identifiable in the CMC-$\theta$ distribution.}}

% \citet{tong_improved_2015} introduce an additional criterion to identify
% a mode in the CMC-\(\theta\) distribution.
% {\textcolor{purple}{We add some mathematical formalism to clarify how this mode is identified.
...
% {\textcolor{purple}{If the angular range is within $T_\theta$, we say that the cartridge case pair "passes" the High CMC criteria. Otherwise, the pair "fails" the High CMC criteria and the CMCs determined under the original method \citet{song_proposed_2013} are used.}}
...
% {\textcolor{purple}{In this section, we demonstrate our implementation of the decision rules applied in the original method of \citet{song_proposed_2013} and the High CMC method of \citet{tong_improved_2015}.
...
% {\textcolor{purple}{\autoref{fig:topVoteCMCPlot} shows the CMCs and non-CMCs determined under the original method of \citet{song_proposed_2013} in blue and red, respectively.
...
% {\textcolor{purple}{Treating Fadul 1-1 and Fadul 1-2 as the reference scan yields 19 and 18 CMCs, respectively.
...
% {\textcolor{purple}{Only 1 cell was classified as a CMC treating Fadul 1-1 as the reference scan under the original method of \citet{song_proposed_2013}. 0 cells were classified as CMCs in the other direction.
...
% {\textcolor{blue}{This section needs to serve as a transition between the method and the investigation - summarizing the holes we've discovered in the descriptions and moving towards a resolution of those holes through brute-force investigation.}}
...
{\textcolor{purple}{This is an issue in any computational research for which only prosaic descriptions of algorithms are provided.}}
{\textcolor{purple}{The only solution to such ambiguity is to enumerate, implement, and pare-down the possible choices that could have been made to arrive to published results.
...
{\textcolor{purple}{We have applied our implementation of these two methods to the}} 780 cartridge case pairs available in the \citet{fadul_empirical_nodate} data set {\textcolor{purple}{under a variety of processing conditions}}.
...
% A sensitivity analysis of the original method {\textcolor{purple}{of \citet{song_proposed_2013}}} and its descendants has yet to be published. 
...
\]
{\textcolor{purple}{where $\overline{\text{CMC}}_i$ denotes the within-group CMC count average and $\overline{\text{CMC}}$ denotes the grand CMC count average. 
...
% {\textcolor{purple}{As an example, \autoref{fig:decisionRuleSensitivity_comparison} compares the CMC results using the original method of \citet{song_proposed_2013} vs. the High CMC method where $T_{\Delta x} = 20 = T_{\Delta y}$ pixels, $T_{\text{CCF}} = .5$, and $T_{\theta} = 6$ degrees. 
...
% {\textcolor{purple}{There are other conclusions that we can draw from \autoref{fig:cmc_sensitivityScatter} that connect to what we have discussed previously as patterns in the CMC literature that make reproducibility impossible. 
...
% {\textcolor{purple}{Changes to the preprocessing procedures have major impact on the effectiveness of the CMC method. 
% For example, consider a change to the procedure described in the}}
% \protect\hyperlink{preProcessing}{preprocessing}
% {\textcolor{purple}{section in which surface matrices are not leveled before applying the bandpass Gaussian filter (i.e., skipping step (3) shown in \autoref{fig:processingPipeline}). 
...
{\textcolor{purple}{\autoref{fig:varRatios} also shows the results for both decision rules where the surface matrices were not leveled prior to applying the bandpass Gaussian filter. 
...
% {\textcolor{purple}{As another example, unintentional ambiguities in written-language description of the High CMC method lead to different interpretations of its decision rule}}.
% \protect\hyperlink{highCMCMethod}{Recall that}
% {\textcolor{purple}{the last step of the method is to combine CMCs from the two comparison directions, assuming that the High CMC criteria were satisfied.
...
% section}){\textcolor{purple}{, it is unclear how to handle such situations.
