...
\section{Cartridge cases \& breech face impressions}\label{sec:cartridgeCases_bfImpressions}
...
% During a forensic examination, two pieces of ballistic evidence are placed under a \textit{comparison microscope}. 
...
\section{The CMC method}\label{sec:cmcMethod}
...
At each step, we compare the description in the published papers with the implementation in code, discussing the gaps in the method description and how we filled in those gaps during the creation of \textbf{cmcR}.\footnote{An \hyperref[sec:example]{example comparison} between two scans is given in the Appendix.}

All CMC methods can be broken down into three broad stages: preprocessing, cell-based similarity feature extraction, and application of a decision rule.\footnote{This pipeline is illustrated in \autoref{fig:overview-flow} in the Appendix.}
...
\subsection{Initial Data}\label{sec:initialData}
...
The \textbf{x3ptools} package \citep{x3ptools} can be used to work with these x3p files in R.\footnote{\autoref{fig:cartridgeCasePair} in the Appendix shows two unprocessed, known match cartridge case scans.}
...
\subsection{Preprocessing procedures}\label{sec:preProcessing}
...
In particular, the preprocessing steps differ considerably across CMC papers.\footnote{These steps are illustrated in \autoref{fig:preprocessing-schematic} and \autoref{fig:processingPipeline} in the Appendix.}
...
\subsubsection{Implementation of preprocessing procedures}\label{sec:preProcessImplement}
...
%The \texttt{preProcess\_removeTrend} functions levels the breech face impression regions. Note that this step is not discussed anywhere in the CMC literature yet has a demonstrable impact final results (as detailed in the \hyperref[sec:investigation]{investigation} section).
% as shown in step (3).
...
\subsection{"Correlation cell" comparison procedure}\label{sec:comparisonProcedure}
...
\subsubsection{Implementation of the correlation cell comparison procedure}\label{sec:comparisonImplement}
...
{\jz{When extracting similarity features, only cells with a proportion of missing pixels below a user-defined threshold are }}{\svp{assessed}} (85\% in our simulations). %\autoref{fig:comparisonCode}).
...
%\autoref{fig:comparisonCode} shows the \texttt{comparison\_allTogether} function applied for each $\theta$ value in sequence, rotating the target scan relative to the reference scan. 
When implementing the High CMC method, both combinations of reference and target scan are examined (e.g. A-B and B-A). 
The result of \texttt{comparison\_allTogether} is a data frame containing similarity features (CCF$_{\max}$, $x$, $y$, $\theta$) used as inputs to the methods for assessment of match strength.\footnote{Example output is shown in \autoref{tab:cellCCF} in the Appendix.}
...
We briefly describe the two {\jz{decision rules}} implemented in \textbf{cmcR}: the original method of \citet{song_proposed_2013} and the High CMC method \citep{tong_improved_2015}.\footnote{For a more thorough explanation of the procedures, refer to the \href{https://csafe-isu.github.io/cmcR/articles/decisionRuleDescription.html}{CMC Decision Rule Description} vignette in the \textbf{cmcR} package.}
...
\subsubsection{The original method of \citet{song_proposed_2013}} \label{sec:originalMethod}
...
\subsubsection{The High CMC method}\label{sec:highCMCMethod}
...
\subsubsection{Implementation of decision rules}\label{sec:decisionRuleImplementation}
...
% \hyperref[sec:investigation]{investigation}
% {\jz{section}}.

The \texttt{decision\_CMC} function applies both the decision rule of the original method of \citet{song_proposed_2013} and the High CMC method depending on whether the user provides a value for $\tau$; the constant used to define "high" CMC values in the CMC-$\theta$ distribution.\footnote{CMCs identified for a match and non-match pair are shown in \autoref{fig:topVoteCMCPlot}, \autoref{fig:highCMCPlot}, and \autoref{fig:knmCMCPlot} in the Appendix.}
...
% \subsection{An incomplete method? XXX this needs a better title XXX}\label{sec:incompleteMethod}
\subsection{Ambiguities in algorithmic descriptions}{sec:ambiguities}
...
\section{Investigation of cell-based forensic pattern matching methods}\label{sec:investigation}

% As described in the \hyperref[sec:initialData]{initial data} section, the set of cartridge case scans from \citet{fadul_empirical_nodate} is commonly used to compare the performance of various methods in the CMC literature. 
...
% For any cartridge case pair, the number of CMCs can be determined under the original method of \citet{song_proposed_2013} and the High CMC method described in \hyperref[sec:implementation]{implementation} section.
{\jz{We have applied our implementation of these two methods to the}} 780 cartridge case pairs in the Fadul data set {\jz{under a variety of processing conditions}}.
Perfect identification of all matching and non-matching pairs is equivalent to choosing a minimum CMC count threshold that separates the distributions of the matching and non-matching CMC counts.
...
% (discussed in the \hyperref[sec:originalMethod]{original method decision rule} section)
...
\subsection{Exploring a method's sensitivity and ambiguity through modularization}\label{sec:exploring}
...
\begin{itemize}[leftmargin=*]
\item the decision rule used,\footnote{\autoref{fig:decisionRuleSensitivity_comparison} in the Appendix shows CMC distributions under the two decisions with other processing conditions held constant.}
...
%\subsection{Exploring algorithmic ambiguities through modularization}\label{sec:exploring}
...
% \hyperref[sec:preProcessing]{preprocessing}
...
% \hyperref[sec:highCMCMethod]{Recall that}
...
% \hyperref[sec:highCMCMethod]{High CMC decision rule
...
%This also demonstrates how a prosaic description of an algorithm is generally insufficient to properly implement the method. 
...
%We have demonstrated that the implementation of the CMC method in the \textbf{cmcR} package is qualitatively similar to those in the CMC literature.
...
Even precise algorithmic descriptions typically gloss-over the usage of particular programming languages, functions, or parameter settings. 
Algorithms that theoretically should be consistent across implementations may in-practice yield numerical inconsistencies due to factors that would require the authors' careful consideration (e.g., how pseudorandom numbers generated).
...
\subsection{Reproducible Example}\label{sec:example}
