...
The \textbf{cmcR} package contains implementations designed for use with 3D topographical scans of the original method described in \citet{song_proposed_2013} and \citet{song_3d_2014} and the High CMC method described in \citet{tong_improved_2015} (\url{https://github.com/CSAFE-ISU/cmcR} for full source code).
...
At each step, we compare the description in the published papers with the implementation in code, discussing the gaps in the method description and how we filled in those gaps during the creation of \textbf{cmcR}.

All CMC methods can be broken down into three broad stages: preprocessing, cell-based similarity feature extraction, and application of a decision rule (see \autoref{fig:overview-flow} in Appendix).
...
In particular, the preprocessing steps differ considerably across CMC papers (see \autoref{fig:preprocessing-schematic} in Appendix).
...
However, the target scan is rotated to determine the rotation at which the two scans achieve the largest \textit{cross-correlation function} value (see \autoref{fig:cmc-schematic} in Appendix).
...
{\jz{When extracting similarity features, only cells with a proportion of missing pixels below a user-defined threshold are }}{\svp{assessed}} (85\% in our investigation). %\autoref{fig:comparisonCode}).
...
We briefly describe the two {\jz{decision rules}} implemented in \textbf{cmcR}: the original method of \citet{song_proposed_2013} and the High CMC method \citep{tong_improved_2015} (see \href{https://csafe-isu.github.io/cmcR/articles/decisionRuleDescription.html}{CMC Decision Rule Description} vignette for a more thorough explanation).
...
% \subsubsection*{An incomplete method? XXX this needs a better title XXX}\label{sec:incompleteMethod}
...
These are similar thresholds to those chosen in other CMC papers (see \autoref{fig:cmc_sensitivityScatter} in the Appendix for variance ratios for both $\theta = 3, 6$ degrees.)
...
%\subsubsection*{Exploring algorithmic ambiguities through modularization}\label{sec:exploring}
